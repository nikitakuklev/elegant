...
void global_trajcor_plane(CORMON_DATA *CM, STEERING_LIST *SL, long coord, TRAJECTORY **traject, long n_iterations, RUN *run, 
...
long zero_correctors(ELEMENT_LIST *elem, RUN *run);
long zero_hcorrectors(ELEMENT_LIST *elem, RUN *run);
long zero_vcorrectors(ELEMENT_LIST *elem, RUN *run);
...
double compute_kick_coefficient(ELEMENT_LIST *elem, long plane, long type, double corr_tweek, char *name, char *item, RUN *run)
{
     ...
     */
                ...
                *((double*)(elem->p_elem+param_offset)) = value;
                ...
                fprintf(stderr, "error: element %s does not have a matrix--can't be used for steering.\n",
                    elem->name);
                exit(1);
                }
            break;
...
long do_correction(CORRECTION *correct, RUN *run, LINE_LIST *beamline, double *starting_coord, BEAM *beam, long sim_step)
{
    ...
    if (correct->prezero_correctors) {
        long changed;
        if (beamline->elem_recirc)
            changed = zero_correctors(beamline->elem_recirc, run);
        else
            changed = zero_correctors(&(beamline->elem), run);
            ...
            for (i_cycle=0; i_cycle<correct->n_xy_cycles; i_cycle++) {
                ...
                if (!y_failed && correct->CMy->ncor) {                    
                    ...
                    if (correct->method==GLOBAL_CORRECTION)
                        ...
                        one_to_one_trajcor_plane(correct->CMy, &correct->SLy, 2, correct->traj+1, correct->n_iterations, run, 
                                                 beamline, starting_coord, (correct->use_actual_beam?beam:NULL));
...
void compute_trajcor_matrices(CORMON_DATA *CM, STEERING_LIST *SL, long coord, RUN *run, LINE_LIST *beamline, long find_only, long invert)
{
    ...
    fprintf(stderr, "computing response matrix...\n");
    report_stats(stderr, "start");
    ...
    if (!do_tracking(one_part, &n_part, NULL, beamline, &p, (double**)NULL, (BEAM_SUMS**)NULL, (long*)NULL,
                     traj0, run, 0, TEST_PARTICLES+TIME_DEPENDENCE_OFF, 1))
    ...
    for (i_corr = 0; i_corr<CM->ncor; i_corr++) {
        ...
        fill_double_array(*one_part, 7, 0.0);
        if (!do_tracking(one_part, &n_part, NULL, beamline, &p, (double**)NULL, (BEAM_SUMS**)NULL, (long*)NULL,
                traj1, run, 0, TEST_PARTICLES+TIME_DEPENDENCE_OFF, 1))
            bomb("tracking failed for test particle (compute_trajcor_matrices())", NULL);
    ...
    *sl_index = NULL;
    ...
    do {
        ...
        if (!(corr = next_element_of_types(corr, SL->corr_type, SL->n_corr_types, &index)))
            break;
        if (steering_corrector(corr) && match_string(corr->name, SL->corr_name, SL->n_corr_types, EXACT_MATCH)>=0) {
            ...
            *kick_coef = trealloc(*kick_coef, sizeof(**kick_coef)*(*ncor+1));
            if (!((*kick_coef)[*ncor] = compute_kick_coefficient(corr, plane, corr->type, 
                ...
                fprintf(stderr, "error: changing %s.%s does not kick the beam--can't use for steering.\n",
    ...
    if (!recircs) {
        ...
        while (moni) {
            if (find_index(moni->type, SL->corr_type, SL->n_corr_types)!=-1 && 
                    (steering_corrector(moni) || match_string(moni->name, SL->corr_name, SL->n_corr_types, EXACT_MATCH)>=0))
                corr_seen = 1;
        ...
        double dp, long start_from_recirc, long fixed_length, double *starting_point, double change_fraction)
{
    ...
    if (!m_invert(INV_ImR, ImR)) {
        fprintf(stderr, "error: unable to invert matrix to find closed orbit!\nThe R matrix is:");
        for (i=0; i<4; i++) {
            fprintf(stderr, "R[%ld]: ", i+1);
            for (j=0; j<4; j++)
                fprintf(stderr, "%14.6e ", R->a[i][j]);
            fputc('\n', stderr);
            }
        exit(1);
        }
    ...
    do {
        ...
        do_tracking(one_part, &n_part, NULL, beamline, &p, (double**)NULL, (BEAM_SUMS**)NULL, (long*)NULL,
            clorb+1, run, 0, TEST_PARTICLES+TIME_DEPENDENCE_OFF+(start_from_recirc?BEGIN_AT_RECIRC:0), 1);
        if (n_part==0)
...
long zero_correctors(ELEMENT_LIST *elem, RUN *run)
{
    ...
    log_entry("zero_correctors");

    while (elem) {
        if (steering_corrector(elem)) {
            switch (elem->type) {
                case T_HCOR:
                    n_zeroed++;
                    ((HCOR*)elem->p_elem)->kick = 0;
                    if (elem->matrix) {
                ...
                case T_HVCOR:
                    n_zeroed++;
                    ((HVCOR*)elem->p_elem)->xkick = ((HVCOR*)elem->p_elem)->ykick = 0;
                    if (elem->matrix) {
                        free_matrices(elem->matrix);
                        tfree(elem->matrix);
                        elem->matrix = NULL;
