    ...
    *((double *)(corr->p_elem + kick_offset)) = kick0 - corr_tweek;
    ...
    for (i_moni = 0; i_moni < CM->nmon; i_moni++) {
      ...
      Mij(CM->C, i_moni, i_corr) =
        (computeMonitorReading(CM->umoni[i_moni], coord, clorb1[i].centroid[0], clorb1[i].centroid[2], 0) - computeMonitorReading(CM->umoni[i_moni], coord, clorb0[i].centroid[0], clorb0[i].centroid[2], 0)) / (2 * corr_tweek);
