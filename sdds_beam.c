    ...
    if (!track_pages_separately) {
      ...
      if ((beam->original == NULL) && !has_been_read) {
        ...
        if (save_initial_coordinates || n_particles_per_ring != 1)
          beam->particle = (double **)czarray_2d(sizeof(double), beam->n_particle = (long)(n_particles_per_ring * beam->n_original * factor), totalPropertiesPerParticle);
        else {
        ...
        if (run->acceptance)
          ...
          if (isSlave || !notSinglePart)
#endif
            beam->accepted = (double **)czarray_2d(sizeof(double), (long)(beam->n_particle * factor), totalPropertiesPerParticle);
        new_particle_data = 1;
      } else
