...
\item In versions prior to 19.0, the $I_5$ radiation integral was computed incorrectly for 
  ...
  W. Guo (BNL) for finding this bug.  Also, the $I_2$ radiation integral (which affects $\alpha$)
  had a sign error.
...
\item {\tt touschekLifetime} --- This program calculates Touschek lifetime using A. Piwinski's formula.
        Input files are generated from ``twiss\_output'' and ``momentum\_aperture''.
\end{itemize}
...
dispersion. User also can use this program to estimate IBS growth rates
for Linac beam, with special attention paid on beam's energy
...
(isRing=1, default) or Linac beam (isRing=0). When isRing is set to 0, the energy scaling
...
A. Piwinski's formula~\cite{Piwinski}.
...
This file can contain a subset of elements of twissFile (for example: only Quadrupole elements). 
But, they {\bf must} point to same beamline. One for a part of beamline (a few sectors) and one for
entire ring is {\bf prohibit}.
\item {\tt -charge}, {\tt -particles} --- Give the charge (in nanocoulombs) or the
 number of electrons.
\item {\tt -coupling} --- Give the emittance coupling ratio, $\epsilon_y/\epsilon_x$.
\item {\tt -RF=Voltage={\em MV},harmonic={\em value}} --- Specify rf voltage and harmonic number.
\item {\tt -length={\em mm}} --- Specify the rms bunch length.
