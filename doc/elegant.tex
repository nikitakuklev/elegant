...
\author{Program Version 17.2.1\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 17.2.1}

This is primarily is bug-fix and performance release, with several significant bugs
being corrected.  Parallelization now includes the longitudinal and
transverse space-charge elements.  Several significant performance enhancements
were made as well.  Here is a summary of what's changed
since release 17.1.1.  For details and references, see the specific
...
\begin{itemize}
\item Output of the inverse response matrix during multi-step runs had a bug, resulting in 
 repeated output of the same data.  Also, it wasn't always possible to get
 get updated response matrix output that was updated during multi-step runs, if the
 fixed path-length constraint was used, for example.  Reported by Michael Abo-Bakr (BESSY).
\end{itemize}

\subsection{Bug Fixes for Elements}
\begin{itemize}
\item The {\tt CWIGGLER} element had an error in the ordering of rotation and displacement removal 
 operations.   Simulations that contained both rotational
 and displacement errors for the same \verb|CWIGGLER| element should be repeated.
\item There was an error in an exponent for synchrotron radiation expressions, resulting
...
\item The correction to the response matrix when there was dispersion at the corrector was being
...
\item When particles were lost on an \verb|ECOL| element with non-zero length, the loss locations
 were recorded incorrectly.  Reported by Feng Zhou (SLAC).
\item The \verb|FTRFMODE| element, which allows simulating multiple tranverse rf modes, had an unitialized
 variable that would result in incorrect accumulation of voltages over many passes.
\item Previously, the transverse space-charge element \verb|SCMULT| could be inserted by hand into the 
 lattice, but it wouldn't actually run.  Now, the user is required to provide a \verb|insert_sceffects|
 command to use \verb|SCMULT|.
\item The longitudinal space charge algorithm (\verb|LSCDRIFT| and \verb|RFCW|) would give give 
...
\begin{itemize}
\item The {\tt CWIGGLER} element, which models a wiggler using Y. Wu's symplectic integration code,
 now optionally includes synchrotron radiation.  Y. Wu assisted in this work by helping with understanding of 
 the internals of his code.
\item The {\tt ZLONGIT} and {\tt ZTRANSVERSE} elements will now recognize bunched beams.  This means 
 one can perform efficient simulations of multibunch beams with short-range impedance effects included.
 At present this feature is experimental and may change in the future.  It is not compatible with parallel execution.
 Contact M. Borland for assistance in using it.
\item The \verb|SCMULT| element has been parallelized and now includes non-linear space charge forces \cite{Xiao2007A}.  \verb|SCMULT|
 has also been parallelized.
\item Longitudinal space charge (via \verb|LSCDRIFT| and \verb|RFCW|) have been parallelized.
\item The longitudinal electric field was added to \verb|FTRFMODE|.  This is typically a very small correction.
\item The ramping option was added to \verb|WAKE| and \verb|TRWAKE|, having appeared previously for \verb|ZLONGIT|, \verb|ZTRANVERSE|,
 \verb|RFMODE|, \verb|TRFMODE|, \verb|FRFMODE|, and \verb|FTRFMODE|.
\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item The optimizer can now access quantities related to the coupled lattice functions computed by
 \verb|coupled_twiss_output|.  
\item The efficiency of the \verb|load_parameters| command was greatly improved through use of a 
 hash table.  This will have a significant impact on loading large files.  One implication of this
 is that the character \verb|#| is no longer allowed in beamline and element names.
\item The load-balancing feature may now be turned off using the \verb|load_balancing_on| control in
 \verb|run_setup|.  This makes results deterministic and can aid, for example, in comparing runs
 that are part of a parameter scan.
\end{itemize}

\subsection{Changes and Additions to Related Programs}

\begin{itemize}
\item The \verb|beamLifetimeCalc| script was modified to fix a bug that would result in an 
 error being issued when the beam energy was changed.
\end{itemize}
...
If the \verb|coupled_twiss_output| command was given with \verb|output_at_each_step=1|,
one may use symbols of the form \verb|<name>#<occurrence>.<property>| where
\verb|<name>| is the name of any \verb|MARKER| element with \verb|FITPOINT=1|.
\verb|<property>| may be one of \verb|betax1|, \verb|betax2|, \verb|betay1|,
\verb|betay2|, \verb|cetax|, \verb|cetay|, and \verb|tilt| (these are
the two beta functions for x and y, the coupled dispersion values for
x and y, and the beam tilt).
