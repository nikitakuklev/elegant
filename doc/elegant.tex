...
\author{Program Version 29.2.0\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
A set of \htmladdnormallink{examples and scripts}{http://www.aps.anl.gov/Accelerator\_Systems\_Division/Operations\_Analysis/downloads/elegantExamples.zip} is available that demonstrates many features of {\tt elegant}.
...
\section{Highlights of What's New in Version 29.2}
...
\item Added the \verb|LEFFECTIVE| parameter for \verb|QUAD| and \verb|KQUAD|, which provides a convenient way
  to change the effective length without changing the adjacent drift spaces.
...
\begin{itemize}
\item Added the \verb|full_grid_output| parameter to the \verb|frequency_map| command.
\end{itemize}
...
\item The script \verb|ionTrapping| was added, providing simple ion trapping calculations for uniform bunch trains.
\end{itemize}
...
closed orbit self-consistently. To address this, the \verb|n_slices| parameter is set to 1 for
tilted bending magnets, which reduces the accuracy of the calculations.
\end{itemize}
