...
\author{Program Version 29.0.0\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 29.0}
...
\item The \verb|analyze_map| command can now determine the nonlinear transport matrix up to third order based on tracking data.
  ...
  are determined for both transport lines and rings. This was requested by Y. Hao (BNL) and L. Yang (BNL).
\item The \verb|correct_tunes| and \verb|chromaticity| commands now include a weighting factor that results in minimization of
  the strength changes in the event that more than two familes are provided for correction.
