...
\author{Program Version 2022.2\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 2022.3}

Here is a summary of what's changed since release 2022.2.  Historical change logs are collected in Section
\ref{sect:changeLog}.
...
\item Z. Duan (IHEP) reported an issue with misalignment methods 1 and 2 for \verb|CSBEND| elements when
  ...
  \verb|TILT| and \verb|ETILT| in combination. This was fixed, but for now the exact method of
  performing misalignments has been disabled. Instead, a method that is linear in the errors is used.
\end{itemize}
...
\item \verb|element_name| --- Name of the element where the lattice will end,  which implies removing all 
  elements downstream of the named element. If the element occurs more than once, the first instance is used.
...
\item \verb|malign_method| --- Allows globally setting the default misalignment method. Three values are recognized.  A
  value of \verb|0| indicates the original misalignment method that is entrance-centered and includes three position
  offsets and roll (\verb|TILT|). A value of \verb|1| indicates the new method \cite{Venturini2021} in entrance-centered
  mode; this includes the possibility of pitch and yaw errors on elements that support it.  A value of \verb|2|
  indicates the new method in body-centered mode; this includes the possibility of pitch and yaw errors on elements that
  support it. User's should note an important difference for elements with non-zero \verb|TILT| parameter: for
  \verb|malign_method=0|, the displacements are performed in the lab frame, while for the other methods the displacements
  are in the tilted magnet frame.
...
Here is a summary of what's changed since release 2022.0.  Historical change logs are collected in Section
\ref{sect:changeLog}.
