...
\author{Program Version 15.1.1\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 15.1.1}

Here is a summary of what's changed since release 14.9.1.  For details
...
\begin{itemize}
\item Added second-order path length matrix elements to the {\tt SOLE}
        element (matrix-based solenoid).  This was suggested by P. Emma (SLAC),
        who provided the expressions.
\item Fixed a bug in the {\tt ECOL} (elliptical collimator), which resulted in
        corruption of vertical particle coordinates when the element had non-zero
        length.  This bug was found by P. Emma (SLAC).
\item Fixed a bug that resulted in potentially undefined results when synchrotron
        radiation was turned on in {\tt KQUAD} or {\tt KSEXT} elements and the
        particle positioncoordinates were identically 0.
\item Fixed error in steering multipole implementation: {\tt elegant} was 
  incorrectly complaining when {\tt bn} was zero, when it should be zero at all times.
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item The units of $R_{21}$ in the ``final'' output file were wrong and have been corrected.
        Added output of the maximum particle amplitudes in each transverse coordinate.
\item Fixed bug that prevented proper saving of a lattice when the {\tt alter\_elements}
command was used with wildcards in the name.
\item Fixed a bug in saving/loading parameters when element division was used on 
        dipoles with non-zero edge angles.
\end{itemize}

\subsection{Modified Elements}
\begin{itemize}
\item Added super-elliptical apertures for {\tt ECOL} and {\tt MAXAMP}.  A super-elliptical
 aperture is somewhere between an ellipse and a rectangle.
\item Added one-sided aperture capability to the {\tt ECOL}, {\tt RCOL}, and {\tt MAXAMP}
        elements.
\item The {\tt SCATTER} element now includes an optional scattering probability.
\item Added a radius factor parameter for longitudinal space charge evaluation to the
  {\tt LSCDRIFT} and {\tt RFCW} element.  The default value for this parameter changes the
  computations from the previous release, and was computed by J. Wu (SLAC).
\item Added ability to control expansion of square roots in canonically-integrated
        elements.  The default is no expansion, as before.
\item The {\tt FRFMODE} and {\tt FTRFMODE} elements now optionally provide output of
        voltage in each mode as a function of time.
\end{itemize}

\subsection{Modified Commands}
\begin{itemize}
\item The {\tt run\_setup} command now optionally accepts the central momentum in MeV/c,
        instead of just $\beta\gamma$.
\item The {\tt tune\_shift\_with\_amplitude} command was improved to provide second-order
        shifts as well as the tune spread for a specified range of amplitudes.
        These quantities may be optimized.
\item Added computation of the minimum and maximum tunes for a specified range
 of momentum offsets. These values may be optimized.
\item Optimization may now include the chromatic derivatives of the beta and alpha Twiss
        parameters.
\item The {\tt load\_parameters} command will now accept a list of input files and will
        optionally ignore non-existent files.
\item The lost-particles file (requested from {\tt run\_setup}), now includes the turn on
        which each particle is lost.
\item Made changes that allow computing closed orbit for the momentum offset
 of the beam distribution and then starting the beam on that orbit.
 The changes are to the {\tt closed\_orbit} and {\tt track\_beam} commands.
\item Optimization terms with zero weight are now ignored, instead of generating an error.
\item Added {\tt disable} control for optimization variables and co-variables.
\item Added {\tt disable} control for {\tt transmute\_elements}.
\end{itemize}

\subsection{New Elements}
\begin{itemize}
\item {\tt LSRMDLTR} --- A laser/undulator beam modulator, using a numerically-integrated
        analytical planar undulator field with a laser beam focused to a waist at the
        center of the undulator.  P. Emma (SLAC) requested this addition and helped with 
        the development.
\item {\tt DSCATTER} --- An element for scattering with a user-specified distribution.
\end{itemize}

\subsection{New Commands} 
\begin{itemize}
\item The {\tt frequency\_map} command was added.  It performs Frequency Map Analysis for
 storage rings.
\end{itemize}

\subsection{Changes and Additions to Related Programs}
With this release, we have begun to supply Tcl/Tk scripts that assist users in
using elegant or do related computations. 
\begin{itemize}
\item A new script, {\tt beamLifetimeCalc} is included that performs
 beam lifetime calculations due to gas scattering and the Touschek
 effect.  It will also perform computations as a function of scanned
 parameters (e.g., rf voltage).
\item A new script, {\tt elegantRingAnalysis} is included that
 performs various analyses for storage rings, including phase space tracking, higher-order
 dispersion, and dynamic aperture.
\item A new script, {\tt fin2param} is included.  It can convert a ``final'' output file
 from an optimization run into a parameter-style file that can be used with {\tt load\_parameters}.
\item {\tt sddsanalyzebeam} now computes the longitudinal emittance.  This was requested by
        J. Lewellen (ANL).
\end{itemize}
...
\begin{itemize}
\item type: setup command.  Actual tracking occurs when the {\tt track} command is given.
        The number of turns tracked is set by the \verb|run\_control| command.
