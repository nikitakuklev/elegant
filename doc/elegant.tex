...
\author{Program Version 15.2\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 15.2}

Here is a summary of what's changed since release 15.1.1.  For details
...
\begin{itemize}
\item Fixed some problems with radiation integral calculations for dipoles
 under some conditions:
 \begin{itemize}
 \item If a dipole was split into several pieces, edge contributions to the
 radiation integrals were inappropriately added at the interior ``edges.''
 This might happen if element division was used, or if the lattice definition
 contained a series of adjacent dipole elements with the same name.
 \item In dipoles with gradients, if 
 $k l = \sqrt{\left|K_1+1/\rho^2\right|} l \ll 1$, 
 the radiation integral calculations were unreliable.  Fixing this necessitated using
 a power series expansion for $k l<0.01$, which may result in some very small changes in 
 radiation integrals (relative to the previous version) for weak dipoles, even if there
 is no gradient.
 \end{itemize}
\item Made a correction to the expression for $I_5$ radiation integral for the
 {\tt WIGGLER} element, having to do with propagation of the beta function.
\item Fixed problem with {\tt ZTRANSVERSE}, which used the wrong value for
 $Z(0)$ for a broad-band impedance.
\item Fixed bugs in {\tt RFDF} implementation for {\tt N\_KICKS}$>1$ and {\tt LENGTH}$>0$.
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item Fixed bug with fiducialization during matrix computations, which
impacted the {\tt final} output from the {\tt run\_setup} command and
the output from the {\tt matrix\_output} command.  For simulations
with acceleration, this would have shown up as incorrect matrix
results in the final output file and the output from the {\tt
matrix\_output} command.  Twiss parameters and tracking were not
affected.  This bug was found by Philippe Piot.

\item Fixed bug in {\tt matrix\_output}, which resulted in incorrect
results when there was acceleration and the starting point for the
matrix was not the start of the beamline.  This bug was found by
Philippe Piot.

\item Fixed a bug in orbit correction ({\tt correct} command): when {\tt
prezero\_correctors} was set (the default), {\tt elegant} was
prezeroing the quadrupole steering parameters {\tt HKICK} and {\tt
VKICK} even if they were not selected as steering elements.  This
wouldn't cause any problem unless the {\tt HKICK} or {\tt VKICK}
parameters of {\tt QUAD} or {\tt KQUAD} elements where given non-zero values. 
This is unrelated to quadrupole misalignment.

\item Fixed a bug that resulted in no calculations when a {\tt
load\_parameters} file had no rows in {\tt change\_defined\_values} mode.

\end{itemize}

\subsection{Modified Elements}
\begin{itemize}
\item The {\tt WIGGLER} element was upgraded:
\begin{itemize}
\item Includes vertical focusing effects.
\item Now allows even or odd number of poles.
\item Wiggler strength can be specified via the K parameter, instead of 
 the radius.
\end{itemize}

\item The {\tt SCRIPT} element now allows creation of particles by the script.
Previously, the number of particles returned by the script had to be
equal to or less than the original number.  This is useful, for
example, in simulating shower creation and transport in a beamline using the {\tt
shower} program.  Also added the {\tt START\_PASS} and {\tt ON\_PASS} parameters
\item Added the {\tt ON\_PASS} parameter to the {\tt CENTER} element, so that
the centering can be applied only on the specified pass through the system.
\item For particle output via {\tt WATCH} elements, the {\tt LABEL} input is now put
in a parameter (``{\tt Description}'') in the output file. 
\item The {\tt BMAPXY} eleement will now use {\tt Bx} and {\tt By} columns (field
in Tesla) if {\tt Fx} and {\tt Fy} (normalized field) are not provided.

\item Upgraded the {\tt ZTRANSVERSE} element.  These improvements were suggested
 by Yong-Chul Chae:
\begin{itemize}
\item Added {\tt WAKES} and {\tt WAKE\_INTERVAL} parameters to allow getting
 wake data in an SDDS file.
\item Added {\tt FACTOR}, {\tt XFACTOR}, and {\tt YFACTOR} parameters,
 which allow multiplying the impedance by factors.
\end{itemize}
...
\begin{itemize}
\item Frequency map analysis now outputs tunes on range from [0, 1],
instead of [0, 0.5].  Louis Emery assisted with this improvement and
provided ideas for the algorithm.
\item One can now use differential and multiplicative mode simultaneously for
{\tt alter\_elements}.

\item Added {\tt minimum}{\em n} and {\tt maximum}{\em n} columns to the {\tt sigma}
output file from {\tt run\_setup}, where {\em n} is 1 to 6.  These columns
give the minimum and maximum amplitude of the corresponding
phase-space coordinate.  Fixed problem with units for {\tt s}{\em
i}{\em j} output.  Fixed a bug that resulted in wrong sigma values in
the output file for single-particle beam.

\item The {\tt transmute\_elements} command can now transmute an
element that has the {\tt LENGTH} parameter into an element that doesn't have
{\tt LENGTH}, as long as the length is zero.

\end{itemize}

\subsection{New Elements}
\begin{itemize}
\item Added the CWIGGLER element, which uses a canonically-integrated 
 wiggler code for general wiggler fields. This code was provided by Ying Wu of
 Duke University.
\item Added the RFTM110 element, which provides simulation of a TM110 deflecting
 cavity, using a sixth-order expansion in $k\rho$, where $k$ is the radial
 wavenumber and $\rho$ is the radius.
\end{itemize}

\subsection{Changes and Additions to Related Programs}
\begin{itemize}
\item {\tt beamLifetimeCalc} now optionally uses the {\tt haissinski} program
...
\item {\tt elegantRingAnalysis} improvements:
 \begin{itemize}
 \item Added collective effects tab with Haissinski equation, IBS, and space-charge
 tune shift.
 \item Added floor coordinate computation to basic ring analysis.
 \item Added printout of radiation integrals for basic ring analysis.
 \item Added optional auto-creation of PNG files for each plot.
 \end{itemize}
\item {\tt haissinski} improvements:
 \begin{itemize}
 \item Added {\tt -harmonicCavity} option, allowing inclusion of a second rf cavity with
 a higher harmonic.
 \item Added {\tt -superPeriod} option.
 \item Modified iteration algorithm to actually honor the convergence criterion,
 resulting in more reliable convergence.
 \end{itemize}
\item {\tt ibsEmittance} improvements:
 \begin{itemize}
        \item Reduced the default target to $10^{-6}$, and made the 
         tolerance 1/100 of the target, to get better convergence when IBS is weak.
        \item Added the rf voltage to the output file.
 \end{itemize}
\item {\tt sddsbrightness} improvements:
 \begin{itemize}
 \item The default method is now actually Dejus' method, consistent with the usage message.
  Previously, default method was Borland's method.
 \item Fixed bug in Dejus method implementation: the value 
 of $\gamma$ was not being used consistently.  Rather, in some places, a 
 fixed energy value of 7 GeV was used.
 \item Fixed a bug that led to multiplication of the current by 1000 for 
 ...
 \item Added feature whereby the parameter {\tt ey0} from the twiss file is 
