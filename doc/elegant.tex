...
\author{Program Version 21.0\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 21.0}

Here is a summary of what's changed since release 20.0.  For details and references, see the specific entries in the manual. 

\subsection{Bug Fixes for Elements}
\begin{itemize}
\item The effect of the \verb|SREFFECTS| element on the closed orbit was incorrect if there was dispersion at the element's location.
 This was fixed, but it results in a discrepancy between the closed orbit and tracking unless care is taken.  As a result, the element
 now allows the user to control inclusion of orbit offsets.  The manual page has additional guidance on using this element correctly.
\item For some elements, {\tt elegant} computes a first-order transfer matrix from tracking in order to allow computation of twiss parameters
  and the like.  This wouldn't work correctly for systems with acceleration.  This has been fixed.
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item A bug was fixed in computation of the \verb|dnux|, \verb|dnuy|, and \verb|diffusion| quantities in frequency map analysis on 32-bit computers.
\item A bug was fixed in computation of corrected emittances  for output to the \verb|run_setup/sigma| and \verb|run_setup/final| files.
  This bug resulted in the emittances being set to zero if the energy spread was zero.
\item Several bugs were inadvertently introduced into orbit and trajectory correction in the last revision.  These were fixed in updated
  releases of version 20.0.  These fixes are also present in Version 21.0.  These bugs were reported by A. Petrenko, R. Fliller (FNAL), and others.
\end{itemize}

\subsection{New and Modified Elements}
\begin{itemize}
\item The \verb|CSBEND| and \verb|CSRCSBEND| elements now use a higher-order magnetic field expansion.  This
  was requested by A. Petrenko.  Small changes in tracking results may be observed.
\item The \verb|CSRCSBEND|, \verb|LSCDRIFT|, and \verb|RFCW| elements now support a high-pass filter in addition
  to the existing low-pass filter.  This was requested by J. Wu (SLAC).
\item \verb|The global_settings| command has several new parameters that allow controlling output to the terminal.
  This resulted from requests by G. Shen (BNL) and others.
\item The \verb|WAKE| and \verb|TRWAKE| elements will now issue a warning if the bunch is considered too short compared
  to the spacing of the wake points.  This was suggested by Y. Kim (PSI).
\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item The \verb|twiss_output| command now computes second-order geometric driving terms.  This was requested
  by W. Guo (BNL), who helped with development and testing.
\item The \verb|global_settings| command now offers several options for reducing or redirecting output that 
  goes to the terminal.  This was a result of requests from G. Shen (BNL).
\end{itemize}

\subsection{Changes to Related Programs and Files}

\begin{itemize}
\item A bug was fixed in {\tt km2sdds}.  The script had trouble scanning RADIA output files with extra spaces between
 columns of data.  This was reported by O. Ferrando.
\end{itemize}
...
\hyperref{{\tt global\_settings}}{{\tt global\_settings}}{}{subsec:globalsettings} & Change global settings.\\ \hline
