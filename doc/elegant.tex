...
\author{Program Version 17.3.1\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 17.3.1}

This release has one significant bug fix plus significant feature
improvements for the {\tt CWIGGLER} element.  There are also several minor
...
\item An error was discovered in Y. Wu's C code version of the integrator for a vertical wiggler, used by 
 the {\tt CWIGGLER} element.  This was confirmed with Y. Wu and corrected.
...
\begin{itemize}
\item The {\tt CWIGGLER} element was given a major upgrade.  The primary new feature is better
 handling of helical devices, including new field expansions appropriate for APPLE-type devices
 and adjustments to match the dispersion in both planes.  Convenience features were added to 
 allow modeling more device types without specifying a harmonic expansion file. 
 More checks are made on user input to
 ensure validity.  The user may request output of the fields as seen by the particles.  
 Radiation integral computations are more accurate and include both field components.
 The linear transfer matrix, used for optics calculations, is computed from tracking and thus
 includes both vertical and horizontal focusing.
\item The {\tt TWISS} element can now perform transformations based on analysis of the
 beam.  Previously, the transformation was based on the twiss parameters from the {\tt twiss\_output}
 command only.  Insignificant changes in results may be observed.
...
\item The lattice parser was changed to allow insertion or omission of underscores in field names
 without penalty.  E.g., {\tt KQUAD,N\_KICKS=4} and {\tt kQuad,nKicks=4} will both be accepted.
\end{itemize}
...
betax = {rpnl 8 2 / pi /}
\end{verbatim}
In this example, \verb|betax| is assigned the value $8/(2 \pi)$, which is computed using
the commandline program \verb|rpnl|.  One possible pitfall with using \verb|rpnl| in this
