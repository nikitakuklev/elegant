...
\item In versions prior to 19.0, the $I_5$ radiation integral was computed incorrectly for 
  {\tt WIGGLER} elements with an even number of poles and for all {\tt CWIGGLER} elements.  Thanks to
  W. Guo (BNL) for finding this bug.  Also, the wiggler contribution to the $I_2$ radiation integral had a sign error,
...
\item If longitudinal space charge simulation was invoked (using {\tt LSCDRIFT} or {\tt RFCW}) but
 the {\tt CHARGE} element was omitted, {\tt elegant} bombed.  This has been fixed. Thanks to P. Williams (Daresbury)
...
\item In past versions, the {\tt RFDF} element had a very strange way of defining the RF phase
  ...
  being simply the phase of the first kick the beam receives.  Thanks to Y. Kim (PSI) for bringing
...
\item The algorithm for the \verb|momentum_aperture| command was changed, to address reproducibility issues
 ...
 consistent results. {\em Unfortunately, this change is not backward compatible.}  Thanks to P. Chou (NSRRC) for
...
because {\tt elegant} was originally a matrix code using $s$ as the
