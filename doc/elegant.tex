...
\author{Program Version 16.0\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 16.0}

Version 16.0 of {\tt elegant} has been extensively modified from
previous versions, although few changes should be noticed by users.
This is largely due to code changes required for parallelization.  A
parallel version will be released in the near future. Users may note
that particles in output files are sorted by particle ID, which was not
the case in past versions.

In addition, several significant new capabilities have been added,
including coupled twiss parameter computation and determination of
s-dependent momentum aperture.

Here is a summary of what's changed since release 15.4.1.  For details
...
\begin{itemize}
\item For the \verb|tune_shift_with_amplitude| command in tracking mode, the results are now
 correct when $\alpha_x$ and/or $\alpha_y$ are nonzero at the starting point.
 This error was pointed out by L. Emery (ANL).
...
\item Improved the {\tt ZTRANSVERSE} and {\tt ZLONGIT} elements, which provide simulation
 of transverse and longitudinal impedances.  The improved version features automatic
 adjustment of number of bins and bin size for broad-band impedances.  This prevents
 unphysical results in the simulation.  This was inspired by an example input file
 sent by Y. Hao (BNL).  On a related note, the manual page for \verb|ZTRANSVERSE|
 was updated to fix errors and add a discussion of preparing impedance input.

\item Added end-pass control for the {\tt WATCH} element.  Hence, one can now specify 
 the pass on which to start and end logging of data.

\item Added end-pass control for the {\tt DSCATTER} element. Hence, one can now specify  the
 pass on which to start and end scattering.

\item The {\tt KQUAD} and {\tt KSEXT} elements now include internal apertures.  These may
 result from the existing {\tt MAXAMP} element, or from the new \verb|aperture_input| 
 command (see below).  This was inspired by a discussion with R. Bartolini (DLS).

\item A simple linear transverse space-charge model is available using the \verb|SCMULT| element.
 This was implemented by A. Xiao (ANL).  One simply inserts one or more \verb|SCMULT| elements into
 the lattice, along with a \verb|CHARGE| element to specify the beam charge.

\item Added second-order matrices to the \verb|RFCA| element in non-body-focus mode.  This
 issue was raised by E. Prat.  This improves computation of dispersion due to a corrector
 in a linac. Note, however, that if body-focusing is used, only the 
 first-order matrix is available.

\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item Added the \verb|momentum_aperture| command, which allows determination of the s-dependent
...
\item Added the \verb|aperture_input| command, which allows specifying s-dependent aperture using
 an SDDS file.  This was inspired by a discussion with R. Bartolini (DLS).

\item Added computation of the coupling integral and emittance ratio (see section 3.1.4.4 of
 \cite{HAPE}).  These are available for optimization using the symbols {\tt couplingIntegral} 
 and {\tt emittanceRatio}.  N.B.: this emittance ratio does not include the effect of 
 vertical dispersion.  This change was suggested by V. Sajaev (ANL).

\item Added computation of coupled twiss parameters and beam sizes,
 using code supplied by V. Sajaev (ANL) based on \cite{Ripken}.  This
 is done with the new \verb|coupled_twiss_output| command.  The
...
\item Added the \verb|match_name| feature to the \verb|twiss_analysis|
 command.  For example, one can now compute and optimize on the
 average $\beta_x$ at elements with names matching a wildcard string.

\item The default number of iterations for orbit correction (\verb|correct| command) 
...
\item Added the \verb|linear_chromatic_tracking_setup| command, which allows tracking
...
\begin{itemize}
\item The program {\tt sddsfindresonances} was added.  It analyzes frequency map analysis data
 to find resonance lines.  The program was written by H. Shang (ANL).
\item Support for s-dependent momentum aperture computation was added to \verb|elegantRingAnalysis|.
 This can be done with many processors at once, and is therefore quite fast.
\item Most computations in \verb|elegantRingAnalysis| now use the second-order concatenated matrix
 for better convergence of close orbits.  This requires setting the default matrix order to 
 2 in these runs.  This should not impact any results as long as the tracking lattice file uses
 kick elements (recommended in any case).
\item The program \verb|sddsurgent| was added.  It performs computations of undulator radiation properties
 using code from the programs URGENT (R. Walker) and US (R. Dejus).  It was written by H. Shang (ANL).
 In addition, a script called \verb|quickSDDSUrgent| was added that provides a basic graphical interface
 to \verb|sddsurgent|.  It was written by X. Jiao (ANL).
\item The program \verb|sddsemitproc| was changed to improve behavior when invalid results are returned from correction algorithms.
