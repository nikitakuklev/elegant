...
\author{Program Version 34.1\\Advanced Photon Source\\Michael Borland, Tim Berenc\\ \date{\today}}
\maketitle
...
scripts}{http://ops.aps.anl.gov/downloads/elegantExamples.zip} is
...
\section{Highlights of What's New in Version 34.1}
...
\begin{itemize}
\item The \verb|BMXYZ| element, which integrates particles through 3D magnetic field maps, now includes misalignment parameters.
...
\subsection{Magnetic field and coordinate conventions}

{\tt elegant} follows MAD \cite{MAD} in magnetic field and coordinate conventions.
In particular, a right-handed coordinate system is used in which $z$ is along the beam
direction, $x$ is to the left, and $y$ is up.

There are many conventions for specifying magnetic fields in terms of a multipole expansion, which leads to
potential confusion.
In the convention established by MAD and followed by {\tt elegant}, positive normal multipole coefficients give positive $B_y$
for $x>0$ and $y=0$. 
...
The quantities $K_m$ and $J_m$ are related to $B_m$ and $A_m$ by
\begin{equation}
K_m = \frac{B_m}{H}
\end{equation}
and
...
J_m = \frac{A_m}{H},
\end{equation}
where $H = p/q$ is the rigidty, $p=mc\beta\gamma$ is the momentum, and $q$ is the charge (with sign).
...
\subsection{Highlights of What's New in Version 34.0}
...
\subsection{Highlights of What's New in Version 33.1.1}
