...
\item Added the \verb|BX| and \verb|BY| parameters to the \verb|BGGEXP| element, to allow imposing a uniform ``external''
  magnetic field. R. Lindberg (APS) suggested this as a solution to problems that may occur when the generalized 
  gradient expansion is used for a dipolar field (e.g., a wiggler) but the fields at the ends do not decay sufficiently
  close to zero.
...
\item The \verb|computeGeneralizedGradients| script (used to prepare data for the \verb|BGGEXP| element) did not work
  ...
  This was reported by forum user Ji\_Li and has been fixed.
...
\item \verb|onePeriod| --- If non-zero, the fields are assumed to cover a {\em single period}, e.g., of a periodic wiggler or
  undulator. 
\end{itemize}
