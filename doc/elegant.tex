...
$(x_{max}*\cos(\theta), y_{max}*\sin(\theta))$, where $\theta$ takes
values from $0$ to $\pi$.   In these modes, the output file contains
a parameter called ``Area,'' which gives the area of the dynamic aperture.

{\bf N.B.: Although ``many-particle'' is the default, we recommend
  using one of the line modes for dynamic aperture.}
