...
\author{Program Version 16.1.7\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 16.1.7}

This is mostly a minor bug-fix release, with one new element ({\tt
ILMATRIX}). The most significant change as of version 16.1.7 is that a
parallel version ({\tt Pelegant}) is supported in this release.  All
single-particle dynamics have been parallelized.

Here is a summary of what's changed since release 16.0.1.  For details
...
\begin{itemize}
\item For the {\tt twiss\_output} command, the previous version gave incorrect results for the
 {\tt couplingIntegral} and {\tt emittanceRatio} values.  In addition, solenoid elements were
 ignored, but are now included.
\item Fixed a bug in the lattice parser that resulted in a crash when a value was missing for a parameter.
\item Previously, the {\tt momentum\_aperture} command would not work correctly for runs with multiple steps.
 This bug was discovered by H. Shang (APS) and has been fixed.
\item Fixed a bug in chromaticity correction when \verb|link_elements| was used.  The chromaticity correction
 matrix was sometimes wrong and the correction would not converge.  This bug was discovered by A. Xiao (APS).
\item Fixed a bug in Twiss parameter output when \verb|link_elements| was used to change element lengths.
 The \verb|s| values in the file were sometimes wrong.
\end{itemize}

\subsection{Bug Fixes for Elements}
\begin{itemize}
\item Added code to ensure that the histogram time span is twice the bunch length for ZLONGIT and ZTRANSVERSE
 elements, to prevent wrap-around issues, although these were very unlikely in previous versions if reasonably
 good frequency resolution was used.
\end{itemize}

\subsection{New and Modified Elements}
\begin{itemize}
\item Added the {\tt ILMATRIX} element, which allows fast tracking in periodic systems using an Individual
 Linear Matrix for each particle.  The matrix has momentum and amplitude dependence via coefficients (e.g.,
 chromaticity, tune-shift-with-amplitude) specified by the user.  This was inspired by requests from Y. Chae (APS).
\item Added start-pass, gradual ramp-on, and high-frequency cut-off filters to the ZLONGIT and ZTRANSVERSE elements.
 These features were requested by Y. Chae (APS).
\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item Added descriptive text to the SDDS files for quantities in the {\tt sigma}, {\tt centroid},
  and {\tt final} output files.
\item Increased the maximum length of the name for an element from 8 to 100 characters.  This was requested
 by R. Bosch (SRC).
\item The {\tt twiss\_analysis} command now creates quantities for the dispersion slope and alpha.
 Twiss analysis is no longer length-weighted, since this doesn't work when averaging over markers. 
 Verbosity control was added to aid in confirming the elements included in the analysis.
\item The {\tt track}  command now has a parameter (\verb|stop_tracking_particle_limit|) that can be
 used to tell {\tt elegant} to stop tracking when the number of particles falls below a preset limit.
\end{itemize}

\subsection{Changes and Additions to Related Programs}

\begin{itemize}
\item The program {\tt touschekLifetime} was added.  It uses Piwinski's formalism to compute the Touschek
 lifetime from {\tt elegant}'s momentum aperture and twiss parameter output.  It was written by A. Xiao (ANL).
\item The GUI {\tt elegantRingAnalysis} now supports addition of errors to many calculations.  The interface was
  also improved using nest tabs.  This was done by H. Shang (APS).
\item The program {\tt sddsurgent} had a bug in the pipe option, which was fixed along with some other bugs.  The maximum size of 
  the grids was increased from 50x50 to 500x500.  This was done by X. Jiao (APS).
\end{itemize}
