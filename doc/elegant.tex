...
\author{Program Version 33.0.0\\Advanced Photon Source\\Michael Borland, Tim Berenc\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 33.0}
...
\item The \verb|load_parameters| and \verb|save_lattice| commands incorrectedly saved the edge angles and other edge-related
  quantities for bending magnets that were reflected. This issue was fixed. Previously-saved parameter files should be modified
  (e.g., remove the edge parameters). This problem was reported by Y. Li (BNL).
\end{itemize}
...
\item Aperture enforcement inside \verb|KQUAD|, \verb|KSEXT|, \verb|KOCT|, \verb|KQUSE|, \verb|CSBEND|, and \verb|CSRCSBEND| elements
  has been improved. In particularly, the \verb|ELLIPTICAL|, \verb|EXPONENT|, \verb|YEXPONENT|, and \verb|OPEN_SIDE| parameters of
  \verb|MAXAMP| are now implemented.
...
\begin{itemize}
\item The \verb|bunched_beam| command can now be set to take desired, fully-coupled bunch parameters from the calculations of the
  \verb|moments_output| command, provided the latter is used to compute equilibrium parameters.
...
These \verb|z| values are assumed to be equispaced and to cover the entire length of the magnet.
