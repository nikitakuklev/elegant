...
\begin{itemize}
\item The \verb|ion_effects| command now supports calculation of ion fields using either a bi-gaussian distribution (sum of two
  gaussians) or a bi-lorentzian distribution (sum of two lorentzians), which allows modeling the core and tails of the 
  distribution more accurately. A number of parameters
...
\item \verb|field_calculation_method| --- By default, the fields are computed on the
  ...
  In that case, the charge distribution is of the form
  \begin{equation}
  ...
  Another method, using a sum of lorentzians, is also available by setting \verb|field_calculation_method| to \verb|"bilorentzian"|;
  this method is considered very experimental at present.
...
\item \verb|fit_residual_type| --- Residual type for distribution fitting. The default is \verb|rms-dev-plus-ad-sum|, which
  ...
  and \verb|rms-dev-plus-centroid|.
...
\item \verb|ion_bin_divisor| --- For \verb|field_calculation_method="bigaussian"| or \verb|"bilorentzian"|, 
  the number of ion bins per rms parameter of  the electron beam.      
\item \verb|ion_range_multiplier| --- For \verb|field_calculation_method="bigaussian"| or \verb|"bilorentzian"|, 
  the full span of the ion binning region bins in units of the rms parameter of the ion distribution.
\item \verb|ion_sigma_limit_multiplier| --- For \verb|field_calculation_method="bigaussian"| or \verb|"bilorentzian"|, 
  the minimum value for either of the
  ion sigmas (for bigaussian) or size parameter (for bilorentizan)
...
\item \verb|ion_histogram_output|, \verb|ion_histogram_output_s_start|, \verb|ion_histogram_output_s_end|, 
      ...
      \verb|field_calculation_method="bigaussian"| or \verb|"bilorentzian"|. 
