...
\author{Program Version 2020.5\\Advanced Photon Source\\Michael Borland, Tim Berenc\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 2020.5}
...
\item {\tt computeGeneralizedGradients} --- Computes generalized gradients from data on a circular cylinder
  for use with the \verb|BGGEXP| element.
\item {\tt computeRBGGE} --- Computed generalized gradients from data on a rectangular boundary 
  for use with the \verb|BGGEXP| element.
...
\item {\bf synopsis:}
\begin{flushleft}{\tt
computeRBGGE -yminus={\em filename} -yplus={\em filename} -xminus={\em filename} -xplus={\em filename}
    -normal={\em outputFilename} [-skew={\em outputFilename}]
    [-derivatives={\em number}] [-multipoles={\em number}] [-fundamental={\em number}]
}\end{flushleft}

\item {\bf switches:}
\begin{itemize}
\item {\tt yplus} --- File containing \verb|x|, \verb|y|, \verb|z|, and \verb|By|, 
  map stored in columns for positive-y plane. If skew components are desired, file must also supply \verb|Bz|.
\item {\tt yminus} --- File containing \verb|x|, \verb|y|, \verb|z|, and \verb|By|, 
  map stored in columns for negative-y plane. If skew components are desired, file must also supply \verb|Bz|.
\item {\tt xminus} ---  File containing \verb|x|, \verb|y|, \verb|z|, and \verb|Bx|, 
  map stored in columns for negative-y plane. If skew components are desired, file must also supply \verb|Bz|.
\item {\tt xplus} ---  File containing \verb|x|, \verb|y|, \verb|z|, and \verb|Bx|, 
  map stored in columns for positive-y plane. If skew components are desired, file must also supply \verb|Bz|.
