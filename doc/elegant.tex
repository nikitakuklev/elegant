...
\author{Program Version 20.0\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 20.0}
...
\item The \verb|ECOL| and \verb|RCOL| elements did not perform correct beam transport computations
  when the length was non-zero and the apertures were both zero.  S. DiMitri (ELETTRA) and 
  M. Cornacchia (SLAC) reported this bug.
\item Previously, \verb|RFCA| and \verb|RFCW| elements with \verb|CHANGE_P0=1| could be treated
 as drift spaces for the purpose of twiss parameter computation by setting \verb|cavities_are_drifts_if_matched=1|
 in \verb|twiss_output|. However, this really doesn't make sense and can give misleading results,
 so it is now disallowed.  This feature should be used only for ring computations when the beam
 energy is not significantly changed by the cavities.  This issue was raised by P. Williams (ASTEC).
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item A bug was fixed for optimization runs when \verb|link_elements| and \verb|save_lattice| were used.
 In particular, the linked elements would not have the correct parameters in the new lattice, although they
 would be correct in the parameter output file (if requested).  This bug was reported by P. Williams (ASTEC).
\item A bug was found in beam moments computations with radiation effects.  The bug, related to
  treatment of edge angles, would have impacted results for cases with significant edge focusing and x-y coupling.
\end{itemize}
...
\item The {\tt UKICKMAP} element now supports synchrotron radiation, both in the radiation integral
 ...
 N.B.: beam moments computations do not include these effects.
\end{itemize}
...
\item Computation of resonance driving terms\cite{Bengtsson} was added to \verb|twiss_output|.  These may included in optimization. 
...
\item The \verb|replace_elements| command was added, which allows replacing a set of elements with a new element,
  as well as removing the elements altogether.
  This is more general than the \verb|transmute_elements|, which permits only a limited set of transmutations.
  This command was added by A. Xiao (ANL).
\item The \verb|global_settings| command was added, which will be used to allow users to control global settings.
  At present, it permits changing the file synchronization behavior.
\item The \verb|find_aperture| command now has an ``n-lines'' mode that permits specifying as many search lines
  as desired.
\item The \verb|moments_output| command has a new flag, \verb|equilibrium| that permits requesting the
  equilibrium moments. Although this was the default before when periodic (``matched'') conditions were invoked, 
  there was no way to request non-equilibrium moments with matched lattice functions.
\item The \verb|include_type_pattern| parameter was added to the \verb|momentum_aperture| command, to permit
...
\hyperref{{\tt global\_settings}}{{\tt global\_settings}}{}{subsec:globalsetttings} & Change global settings.\\ \hline
