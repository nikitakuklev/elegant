...
\author{Program Version 34.4.0\\Advanced Photon Source\\Michael Borland, Tim Berenc\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 34.3.0}
...
\item When used for longitudinal feedback, the \verb|TFBDRIVER| element now includes simulation of the 
  feedback cavity resonance and driving circuit, usign a circuit model developed by T. Berenc (APS).
\item When K. Hwang's fringe model is used for the \verb|CSBEND| element, automatic correction of the \verb|FSE|
  value can optionally be computed in order to null out trajectory errors that result from the fringe fields extending outside
