...
\author{Program Version 15.3\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 15.3}

Here is a summary of what's changed since release 15.2.  For details
...
\begin{itemize}
\item Fixed error in creating the matrix for {\tt RFCA} elements when the {\tt SRS}
 focusing model was invoked.  The $R_{5j}$ matrix elements of the downstream system
 (reported in the {\tt final} output from {\tt run\_setup} or the output of 
 {\tt matrix\_output}) would be incorrect.  Tracking was not affected by this 
 bug, which was discovered by M. Cornacchia.
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item Fixed a bug that occurred when optimization of twiss parameters was performed
 and the {\tt matched\_to\_cell} feature of {\tt bunched\_beam} was used.  The
 bug resulted in twiss variables not getting defined.  This bug was discovered by
 V. Sajaev.
\item Fixed bug in accounting for lost particles for the {\tt losses} output file of
 {\tt run\_setup}.  The problem would be seen when particles were created by a 
 {\tt SCRIPT} element on multiple turns.
\item Multi-step runs (i.e., {\tt n\_steps}>1 in {\tt run\_control})
can now be done with optimization.  This bug was found by P. Emma.
\item Fixed bug in {\tt slice\_analysis} command.  If the {\tt final\_values\_only} parameter
 was nonzero and if a {\tt CSBEND} element was in the lattice, {\tt elegant} would exit with
 a cryptic error.  This bug was found by X. Zhu.
\end{itemize}

\subsection{Modified Elements}
\begin{itemize}
\item The {\tt SCRIPT} element now has a {\tt DIRECTORY} parameter.  If given, input
 and output files are created and retrieved from the specified directory.  For example,
 to increase speed on a cluster one can use {\tt DIRECTORY=/tmp}, forcing the script mechanism to
 use the local temporary drive.
\item Added the {\tt CUTOFF} parameter to {\tt SREFFECTS}, allowing restriction of the number 
 of $\sigma$'s used for the random numbers.
\item Added length, angle, and tilt parameters to the {\tt EMATRIX}
(Explicit MATRIX) element.
\item {\tt MARK} elements with {\tt FITPOINT=1} now produce {\tt rpn} variables with the
 $\Sigma_{ij} = \langle x_i x_j \rangle$ parameters of the tracked beam. The variables are
 named {\em markName}\verb|#|{\em occurrence}.{\tt s}{\em i}{\em j}.
\item Added calibration factors for correctors embedded in quadrupoles ({\tt QUAD} and {\tt KQUAD}).
\end{itemize}

\subsection{Modified Commands}
\begin{itemize}
\item Added the {\tt final\_pass} control to {\tt run\_setup}.  If nonzero, the {\tt centroid}
 and {\tt sigma} output is computed for the final pass only.  By default, these outputs give
 statistics over all passes.  

\item The lattice file may now contain {\tt rpn} sequences.  For example, if you need to define some
physical, mathematical, or other constants for use in element definitions, you may use the following
syntax
\begin{verbatim}
! Define pi
% 1 atan 4 * sto Pi
...
\end{verbatim}
(Note that {\tt pi} is already defined in the standard {\tt defns.rpn}
file.)  This feature was suggested by M. Cornacchia.
\item The ``\%s'' syntax for the rootname may now be used in the input files of {\tt sdds\_beam}.

\item Orbit and tune correction will, if set up, take place inside the optimization loop.

\item Closed orbit values at monitors are now stored in {\tt rpn} variables for use in optimization, provided
 the {\tt CO\_FITPOINT} parameter of the {\tt MONI}, {\tt HMON}, or {\tt VMON} element is set to 1.
 The variable names are\\ {\em bpmName}\verb|#|{\em occurence}.{\em plane}{\tt co}, where {\em plane} is {\tt x} or
 {\tt y}.  This also works for {\tt MARK} elements if {\tt FITPOINT=1} is set.  In addition to {\tt xco} 
 and {\tt yco}, one may refer to {\tt xpco} and {\tt ypco} to get the slopes.

\item Response matrix values (computed by {\tt
 correction\_matrix\_output}) are now stored in {\tt rpn} variables
 for use in optimization.  The variable names are of the form \\ {\em
 Plane}{\tt R\_}{\em bpmName}\verb|#|{\em occurence}{\tt \_}{\em
 corrName}\verb|#|{\em occurence}{\tt .}{\em corrParam}, where {\em
 Plane} is {\tt H} (horizontal) or {\tt V} (vertical) and {\em
 corrParam} is the parameter of the corrector used for changing the
 orbit (e.g., {\tt HKICK} or {\tt VKICK} for a {\tt KICKER} element).
...
\item {\tt EDRIFT} --- This is an Exact DRIFT space.  In particular, path length computations
 are good to all orders in $x^\prime$ and $y^\prime$.

\end{itemize}

\subsection{Changes and Additions to Related Programs}
\begin{itemize}
\item Added the program {\tt sddsemitproc}.  This is a simplified version of {\tt sddsemitmeas}.
 Both programs analyze emittance measurement data from multi-quadrupole scans.  {\tt sddsemitproc}
 is preferred as it lacks ``features'' of {\tt sddsemitmeas} that are not in fact useful.
\item {\tt sddsanalyzebeam} can now generate a new beam file matching the six-dimensional
 phase space of the beam it is analyzing.  This allows, for example, creating a new beam
 file with more particles than the one analyzed.  This is done with the {\tt -generate}
 option.
\item {\tt elegantRingAnalysis} collective effects calculations now use cluster resources if
 requested.
\item {\tt haissinski} now accepts negative values for the harmonic cavity voltage, to allow
 simulation of a bunch lengthening cavity.
\end{itemize}
...
\item \verb|mode| --- May be ``many-particle'', ``single-particle'', ``one-line'', ``three-line'',
``five-line'', ``seven-line'', ``nine-line'', or ``eleven-line''.  Many-particle searching is
much faster than single-particle, but does not allow interval splitting to search for the aperture boundary. 
Both ``many-particle'' and ``single-particle'' modes involve searching from the outside inward, which
improves speed but may result in including islands.
The {\em n}-line modes avoid this by searching form the origin outward.  In these modes, $n$ lines are
explored from $(0,0)$ to $(x_{max}*cos(\theta), y_{max}*sin(\theta))$, where $\theta$ takes values
from $0$ to $\pi/2$. The exception is one-line mode, where $\theta=$pi/4$.
...
\item \verb|search_output| --- The (incomplete) name of an SDDS file for output of detailed
 ...
 ``\%s.apso''.  Valid for many- and single-particle modes.
...
\item \verb|simplex_pass_range_factor| --- When starting a new pass, the simplex optimizer takes
 ...
 as the starting step size for that variable.
