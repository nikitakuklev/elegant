...
\author{Program Version 26.0.1\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 26.0.0}
...
\item The \verb|BUMPER| element now uses the kicker waveform when a kickmap is supplied. Previously, the kicker waveform got ignored when a 
  kickmap was provided.
\item The \verb|BUMPER| element now behaves as expected when the length is zero.
...
\item The steering multipoles feature of \verb|KQUAD| did not include the skew components (driven by vertical steering) when the steering multipoles 
  were specified without the dipole term. 
\end{itemize}
