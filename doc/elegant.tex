...
\author{Program Version 25.2.1\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 25.2.1}

Here is a summary of what's changed since release 25.1:

\subsection{Bug Fixes for Elements}

\begin{itemize}
\item Corrections were made to the nonlinear fringe fields for \verb|KQUAD| elements, thanks to N. Carmignani (ESRF).
  Prior simulations using this feature should be repeated to assess the impact of these corrections.
  Also, the execution speed was optimized.
\item The \verb|CORGPIPE| element had an error in the equation  for the wake, which made negligible changes in the
  results.
\item The \verb|SREFFECTS| element was not being properly initialized for runs with momentum ramping. This bug was
  introduced in a recent version.
\item The default number of bins for LSC calcuations was an odd number, which resulted in an error message since an
  even number was required. The default was changed. J. Lewellen (LANL) pointed out the problem.
\item The lattice parser will now allow comments to be inside the definition of an element, i.e., at the end of the line.
  L. Yang (BNL) pointed out the issue.
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item The \verb|touschek_scatter| command was using the minimum of the absolute values of the positive and negative
  momentum aperture, rather than using both the positive and negative values. This resulted in pessimistic calculations
  of the Touschek lifetime.
\item The chromaticity command \verb|chrom| now performs a minimum of one correction step to ensure tht 
  values are stored in variables used by \verb|optimize| command.
\item There was a bug in the \verb|ramp_elements| and \verb|modulate_elements| commands that resulted in problems
  handling multiple elements with the same name. This was fixed.
\item Closed orbit correction was not respecting the closed orbit iteration parameters when computing the response matrix
  from closed orbits. This bug was pointed out by V. Sajaev (ANL).
\end{itemize}

\subsection{New and Modified Elements}
\begin{itemize}
\item The \verb|BUMPER| element (time-dependent kicker) now supports a kick-map-like feature, so the deflection as a function of x and y can
  be specified.
\item The \verb|RFDF| element (rf deflector) now allows specification of the transverse variation using a sextupole-like term.
\item RFCA/RFCW
\item The \verb|RFMODE|, \verb|TRFMODE|, \verb|FRFMODE|, and \verb|FTRFMODE| have two new features: the ability to
  retain voltage from a previous simulation step, and the ability to suppress same-turn effects on the beam in order
  to allow combined use with short-range wake elements.
\item \verb|SOLENOID| elements may now be subdivided automatically using the \verb|divide_elements| command or the
  \verb|element_divisions| parameter of the \verb|run_setup| command.
\item Added ponderomotive axial focusing for the \verb|TWLA| element, at the request of J. Lewellen (LANL). This feature is
  experimental, so please report any issues to me.
\item Coordinate transformations used by the \verb|CSBEND| and \verb|CSRCSBEND| elements were recoded to simplify the expressions
  and improve accuracy. Very small changes in results may be observed in single-pass systems. In rings, small changes in the location 
  of stability boundaries may be observed; the new version is considered more accurate.
\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item The \verb|frequency_map| command now supports quadratic spacing of the points in x and y.
\item The \verb|optimize| command now accepts the name of an ``interrupt file.'' If this file is modifed or
  created while the optimizer is running, the optimizer will complete on-going operations and then terminate
  cleanly.
\item The \verb|rf_setup| command now has an output file that records the rf parameters, synchrotron tune,
  bunch length, and other quantities. This was requested by H. Owen (ASTeC).
\item The \verb|magnets| output of the \verb|run_setup| command now includes profile data for
  \verb|SOLENOID| and \verb|MAP_SOLENOID| elements.
\end{itemize}

\subsection{Changes for Parallel Version Only}

\begin{itemize}
\item A bug was fixed that prevented proper output of population data during parallel optimization.
\item If the \verb|load_balancing| parameter of the \verb|run_setup| command is set to -1, the load balance is checked and reported
  during tracking, but no rebalancing takes place.
\end{itemize}

\subsection{Changes to Related Programs and Files}

\begin{itemize}
\item The \verb|touschekLifetime|  program  was using the minimum of the absolute values of the positive and negative
  momentum aperture, rather than using both the positive and negative values. This resulted in pessimistic calculations
  of the Touschek lifetime. This was reported by forum user Adrianea.
\item The \verb|sddsfluxcurve| program now supports helical devices. In addition, computation of central cone flux now
  ignores the electron beam dimensions, as is conventional.
\end{itemize}
