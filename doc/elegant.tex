...
\item The \verb|correct_tunes| command now accepts limits on the strength of the quadrupoles.
\end{itemize}
...
\item \verb|quadrupoles| --- List of names of quadrupoles to be used.  Several names may be given and the names may include
  ...
  introduced errors). If that's not the case, the iteration may fail to converge
\item \verb|lower_limit|, \verb|upper_limits| --- Optional lists of lower and upper limits for quadrupole strengths,
  parameter to the \verb|quadrupoles| list. This is an unsual input format for {\tt elegant}, involving a list of
  numbers stored in a string. An example:
\begin{verbatim}
&correct_tunes
 quadrupoles = "Q1 Q2 Q3"
 lower_limits = "0 -1.5 0"
 upper_limits = "1.5 0 1.5"
 ...
&end
\end{verbatim}
