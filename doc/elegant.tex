...
\author{Program Version 14.8\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 14.8}

Here is a summary of what's changed since version 14.7.1.  For details
and references, see the specific entries in the manual.

One notable change is that {\tt elegant} now runs on MAC OS-X, thanks to
the efforts of Robert Soliday (ANL).  Also thanks to Bob's efforts, 
{\tt elegant} is now distributed under an Open Source license.

\subsection{New Elements}

\begin{itemize}

\item Added {\tt SCRIPT} element to allow transforming the beam with an
external script.  For example, the external script might run the
{\tt shower} program to simulate interaction with matter.

\item Added {\tt WIGGLER} element for damping wiggler simulation.  Does not
include edge focusing and has had limited testing.

\end{itemize}

\subsection{Improvements and Bug Fixes for Elements}
\begin{itemize}
\item Added fiducialization to {\tt MATR} element to resolve problems with
        matrices that have nonzero $R_{n5}$ or $T_{n5m}$ elements.
        This bug was found by Paul Emma (SLAC).
\item The {\tt SREFFECTS} element (which does synchrotron radiation 
        effects in tracking) had several bug fixes.  
        \begin{itemize}
        \item Fixed problems with quantum excitation when the element was placed in 
        a region with high dispersion.
        \item Fixed problems with quantum excitation when the 
        of {\tt FRACTION} parameter was used.
        \end{itemize}
\item Fixed bugs and added features to {\tt IBSCATTER} (intrabeam 
        scattering):
        \begin{itemize}
        \item Added {\tt SMOOTH} parameter to allow selecting a nonrandom
        simulation of IBS (simply does inflation of the emittances according
        to the growth factor).
        \item Added {\tt FACTOR} parameter to allow multiplying IBS effects by
        a chosen factor.
        \item Fixed problem with energy scattering that
        caused false exponential growth in energy spread for low-energy beams.
        \end{itemize}
\item Fixed problem with \verb|tlimit| implementation for {\tt CLEAN}
         element that caused incorrect results for low-energy beams.
\item In prior versions, when tracking a circular machine with CSR in
        bends and drifts, the first CSR drift would not be handled properly.
        This has been fixed.
\item Added {\tt LINEARIZE} control for 
        {\tt CSRCSBEND} and {\tt CSRDRIFT}, which allows using purely
        linear transport between CSR wake kicks.
\item Added low-pass filter capability for {\tt CSRCSBEND} element.
        This allows applying a low-pass filter to the density histogram,
        as a form of noise control.  The filter is also applied to the
        {\tt CSRDRIFT} elements downstream of each {\tt CSRCSBEND}
        element.
\item Added renormalization of density histograms after smoothing and filtering
        in {\tt CSRCSBEND} and {\tt CSRDRIFT}.  This enforces conservation of charge
        and prevents negative values of the particle density.
\item Added {\tt LINEARIZE} control for {\tt RFCA} and {\tt RFCW},
        which results in a linear dependence of energy gain on
        phase deviation.
\end{itemize}

\subsection{New Commands}
\begin{itemize}
\item Added {\tt semaphores} command to provide better semaphore functionality.
Previously, one could use the {\tt semaphore\_file} entry on the {\tt run\_setup}
command to cause {\tt elegant} to create a file when the run terminated successfully.
The {\tt semaphores} command improves on this, allowing specification of files
that mark the start and finish of a run.  The {\tt semaphore\_file} entry on the {\tt run\_setup}
command is still functional.
\end{itemize}

\subsection{Modified Commands}
\begin{itemize}
\item For \verb|bunched_beam|, added a new beam type, ``halo(gaussian)'', which
        ...
        for most beam types are different in this version of {\tt elegant}.
\item For \verb|slice_analysis|,  added \verb|final_values_only| control.
        If set, this directs {\tt elegant} to perform slice analysis only
        ...
        \verb|final_values_only| is set).
\item For \verb|load_parameters|, added \verb|include_type_pattern| and
        \verb|exclude_type_pattern| controls.  These allow providing a
...
\begin{itemize}
\item Particle output files from the {\tt WATCH} element now include the
        Charge parameter.
\item Symbol {\tt sTotal} is now available in optimization equations, providing 
 the total arc length of the system.
\item Parameters {\tt DtPercXX} added to {\tt final} output file (from
 \verb|run_setup|), providing percentile locations for time coordinates.
\item Fixed problem with setting of the {\tt optimized} parameter in the 
        {\tt final} output file at the end of optimization.
\end{itemize}
...
computed using the concatenated third-order matrix.  Since (with the
exception of alpha magnets) {\tt elegant} doesn't have matrices higher
than second-order for individual elements, these results are probably
not very reliable.  That is, while they are reasonably accurate in
characterizing the concatenated system of second-order matrices, that
is the limit of their applicability.  Users who are concerned about
these effects should perform off-energy tracking using canonical
elements (i.e., CSBEND, KQUAD, KSEXT, and MULT), which include energy
dependence to all orders.
...
\item {\bf -cosy} --- Provide data for the program COSY INFINITY.  This can take a little while
  ...
  {\tt HGAP} values for all the dipoles.
