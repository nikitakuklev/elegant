...
\item \verb|long_dist|, \verb|tran_dist|,\verb|full_dist| --- The output file name of corresponding longitudinal (2D), transverse (4D) and full dimension histogram analysis. Has to be given together with the corresponding bin values. The output SDDS file contains a column ``weight'' (number of particles at each grid point). The data is sorted in a n-bit counter ($|nx,nxp,ny,nyp,nt,np|$ for 6D). Each page is for each required histogram analysis occurence. 
\item \verb|long_bins|, \verb|tran_bins|, \verb|full_bins| --- The bin number in the order of (x,xp,y,yp,t,p) for doing histogram analysis. Has to be given together with the corresponding output file name.
\item \verb|name| --- Possibily wild-card containing string specifying the
	elements where the histogram analysis is taking place.
\item \verb|type| --- Possibily wild-card containing string specifying the
	element types where the histogram analysis is taking place.
\item \verb|exclude| --- Possibily wild-card containing string specifying 
	elements to be excluded from the specified histogram analysis.
\end{itemize}
