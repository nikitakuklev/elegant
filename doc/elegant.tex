...
\author{Program Version 18.1.3\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 18.1.3}

This release has a major new feature for storage ring simulation,
namely, computation and optimization of fully coupled beam moments in
the presence of synchrotron radiation and damping.  There are also
several some significant bug fixes as well as numerous feature improvements.
The \verb|CWIGGLER| element in particular was much improved.
The set of examples runs has also been expanded.

Here is a summary of what's changed since release 17.2.2.  For details
...
\item Twiss output contains entries for the higher-order dispersion, tune
        shifts with amplitude, higher-order chromaticity, and tune spreads
        due to chromaticity and amplitude 
	{\em even when these are not calculated}, which is potentially
	misleading.   The values are zero when the calculation is not
        requested.
\item Parallel efficiency can depend strongly on the particular problem and
  the amount of outupt.  For example, a simulation without collective effects and
  with minimal output will show excellent scaling up to 500 or 1000 processors, while 
  a simulation dominated by collective effects with frequent output may roll off
  with only 30 processors.  Work continues to improve the scaling. 
\end{itemize}

\subsection{Bug Fixes for Elements}
\begin{itemize}
\item An error was discovered in Y. Wu's C code version of the integrator for a vertical wiggler, used by 
 the {\tt CWIGGLER} element.  This was confirmed with Y. Wu and corrected.  Simulations with horizontal field
 components should be repeated to check results.
\item An error was fixed in field computations for vertical wigglers that had very small effects on
 synchrotron radiation modeling.  
\item A memory access error was discovered in the low-pass filtering routine used by {\tt ZLONGIT} and
 {\tt ZTRANSVERSE}, which would cause a crash on some recent Linux systems.  This was corrected. 
 Very small changes have been observed in some results.
\item A bug was fixed in handling sequences of \verb|CSBEND| elements with the same name with no separating
 elements (or none other than markers).  The edge effects were included between the dipoles, which is incorrect.
 This bug was reported by Y. Roblin (TJNAF).
\item If the bending radius of a \verb|CSBEND| or \verb|CSRCSBEND| exceeds $10^6$ m, it is treated as a drift space to
 prevent numerical problems.  This bug was found in a lattice provided by J. Wu (SLAC).
\item The matrix for the \verb|RFCA| element was incorrect when the phase was different from 0 to 180 degrees and
 the cavity was {\em not} used for acceleration.    This might have affected some storage ring lattice computations.
\item The \verb|GROUP| parameter, used for assigning elements to named groups, now appears in the dictionary lists.
\item Fixed bugs in creating histograms with \verb|HISTOGRAM| when all particles were lost on one processor.  The
 tolerance on the range was increased to avoid dropping particles.  This was done by Y. Wang (APS).
\item Fixed problem with output when the \verb|START_PASS| is not zero for dumping \verb|WATCH| parameters.
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item A bug was fixed in chromaticity correction that resulted in incorrect reporting of maximum $K_2$ values.  Another bug
 was fixed that sometimes resulted in nonconvergence of chromaticity correction when \verb|link_elements| was used in an 
 unusual way (namely, when the baseline value feature on the stack was used).
\item When trajectories are corrected in one-to-one mode using \verb|correct|, the program now uses the first BPM after
 each corrector.  Previously, it might use the wrong BPM if the number of BPMs was greater than the number of correctors.
 Also, in this mode the response matrix is no longer inverted, which saves time.  
\item For \verb|insert_sceffects|, a memory allocation bug that sometimes caused a crash was fixed.  This was found and fixed
 by A. Xiao (APS).
\item The command parser was modified to permit execution of expressions in the rpn calculator module for assignment 
 to command parameters.  This is done by enclosing the expression in parenthesis.  See section 5 for details.
 This was implemented by R. Soliday (APS).
\end{itemize}

\subsection{New and Modified Elements}
\begin{itemize}
\item The {\tt CWIGGLER} element was given a major upgrade.  The
 primary new feature is better handling of helical devices, including
 new field expansions appropriate for APPLE-type devices and
 adjustments to match the dispersion in both planes.  Convenience
 features were added to allow modeling more device types without
 specifying a harmonic expansion file.  A check was added for
 insufficient number of integration steps compared to field harmonics.
 The user may request output of the fields as seen by the particles.

 It is now possible to perform parameter scans, for example of the
 field strength, using \verb|vary_element|.  User may now specify an
 output filename for field samples.  The linear transfer matrix, used
 for optics calculations, is computed from tracking and thus includes
 both vertical and horizontal focusing.  Radiation integral
 computations (for output by the \verb|twiss_output| command) are more
 accurate and include contributions to $I_2$ and $I_3$ from horizontal fields.

\item The {\tt TWISS} element can now perform transformations based on analysis of the
 beam being tracked.  Previously, the transformation was based on the twiss parameters from the {\tt twiss\_output}
 command only.  Insignificant changes in results may be observed in other modes due to rearrangement of the code.
\item Longitudinal space charge is now available for the {\tt RFCW} element even when the SRS body focus
 model is used.  This applies to the parallel and serial versions.
\item Elements of type {\tt CSBEND}, {\tt KQUAD}, and {\tt KSEXT} are now splittable using the \verb|divide_elements| command and the
 \verb|element_divisions| control in \verb|run_setup|.

\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item The \verb|moments_output| command was added, which allows computation, output, and optimization of the results
 of beam moments propagation.  Synchrotron radiation and damping may optionally be included. One may find closed solutions
 (for rings) or simply propagate moments from a defined starting point.
\item The \verb|alter_elements| command now has several features that allow restricting the application of the alteration,
 based on position, occurence number, and position relative to other elements.  This was requested by V. Sajaev (APS).
\item The {\tt error\_control} command now accepts an overall error factor that is applied to all
 error amplitudes.
\item The \verb|optimization_term| command now includes the ability to create a sequence of terms by substitution of a
 sequence of numerical values into the term definition.  This uses the new \verb|field_string| feature.
\item The lattice parser was changed to allow insertion or omission of underscores in field names
 without penalty.  E.g., {\tt Q1: KQUAD,N\_KICKS=4} and {\tt Q1: KQUAD,NKICKS=4} will both be accepted in the lattice file.
 Note that in other contexts, such as altering elements, adding errors, or loading parameters from an SDDS file, one must
 still observe the correct spelling as given in the element dictionary.
\item When lattices are saved with \verb|save_lattice|, the element and beamline definitions are now saved in the 
 same order as in the original file. 
\item The \verb|correct| command (for orbit or trajectory correction)
 now has a parameter that specifies the minimum number of x-y cycles
 to perform before giving up.
\end{itemize}

\subsection{Improvements and Bug Fixes for Parallel Version}

This work was performed by Y. Wang (APS).
\begin{itemize}
\item It is now possible to run in parallel mode even when the first tracking step is set to fiducialization using
 \verb|first_is_fiducial=1| in \verb|run_control|. 
\item Improved memory usage efficiency to allow {\tt Pelegant} run larger simulations.  We've run up to 60 million particles
 for the FERMI@ELETTRA FEL driver using this version.   The current memory bottleneck on master will be
 eliminated in the next version with parallel I/O.
\item Parallelized longitudinal space charge when used as part of an \verb|RFCW| element.
\end{itemize}
...
\begin{itemize}
\item The set of example runs has been significantly expanded.  These are distributed as a separate file.
\item The program {\tt ibsEmittance} was revised to forestall confusion by adding more data to the output file.
\end{itemize}

\section{Credits}

Contributors to {\tt elegant} include M. Borland, R. Soliday, V. Sajaev, Y. Wang, Y. Wu, and A. Xiao.
Contributors to related programs include M. Borland, R. Dejus, L. Emery, and H. Shang.
...
simulation of collective effects, such as wakefields, resonator
impedances, intra-beam scattering, coherent synchrotron radiation, and
the longitudinal space charge impedance.  Some of the more unusual
elements available are third-order
alpha-magnets\cite{Borland_thesis,Enge}, time-dependent kicker
...
optimization.  It will fit the first- and second-order matrix, beta
functions, tunes, chromaticities, natural emittance, etc.  It also has
the ability to optimize results of tracking using a user-supplied
function of the final beam and transport parameters.  This permits
solution of a wide variety of problems, from matching a kicker bump in
the presence of nonlinearities to optimizing dynamic aperture by
adjusting sextupoles.
...
\end{latexonly}
\subsection{coupled\_twiss\_output}
...
\item \verb|xmin|, \verb|xmax| --- Limits of grid of initial x coordinates for tracking.             
 \verb|xmin| should typically be a small, positive value so that there                               
...
\end{latexonly}
\subsection{insert\_sceffects}

\begin{itemize}
\item type: setup command.
\item function: set up for transverse space charge calculation.
{\bf N.B.: This feature is not designed for space charge
...
\item \verb|name| --- Possibily wild-card containing string specifying the
	elements name. After that, the space charge kick is to be inserted.
\item \verb|type| --- Possibily wild-card containing string specifying the
	element types. After that, the space charge kick is to be inserted.
\item \verb|exclude| --- Possibily wild-card containing string specifying 
	elements to be excluded from the specified space charge kick insertion.
...
\item \verb|element_prefix| --- give space-charge kick name. Will apear as element name in beamline.
\item \verb|skip| --- If nonzero, number of ``skip'' space charge kicks are skiped. 
        If zero, only one space charge kick is inserted to the end of beamline. 
\item \verb|vertical|, \verb|horizontal|, \verb|nonlinear| --- Switches for setting up 
space charge kick calculation.
\item CHARGE --- User has to use CHARGE to set up bunch current. 
\item Note --- Some user
use  $``matched\_to\_cell''$ or $``use\_twiss\_command\_values''$ inside $``bunched\_beam''$
command. Those options will erase SCMULT assignment along beamline. In this case, user has to re-calculate twiss using $``twiss\_output''$ just before tracking.
\end{itemize}
...
\end{latexonly}
\subsection{linear\_chromatic\_tracking\_setup}
...
\item \verb|oversteps| --- Number of step sizes to go past the point of first
...
\item \verb|steps_back| --- When performing interval splitting, when a particle is
...
In {\tt elegant}, the algorithm is as follows.  For simplicity in
...
\begin{enumerate}
\item Choose a large momentum deviation $\delta$ (e.g., 10\%) that is sure to be lost, and track.
\item Decrease $\delta$ in fairly large steps $\Delta\delta_0$ (e.g., 1\%) until the particle survives.
 Call this momentum deviation $\delta_{s1}$ and the previous value (where the particle was lost)
 $\delta_{l1}$.
\item Scan outward from $\delta_{s1}-J\Delta\delta$, where the integer $J$ is the ``overstep'' amount,
 typically 1.
 Use a step-size of $\Delta\delta_1 = \Delta\delta_0/D$, where $D$ is an integer (typically 10), 
 not exceeding $\delta_{l1}$.  Stop when a loss occurs.  Call this momentum deviation
 $\delta_{l2}$ and the last surviving value $\delta_{s2}$.
\item If accuracy is acceptable, stop at this point.  The momentum aperture is $\delta_{s2}$.
\item If greater accuracy is desired, ``step back'' to a momentum deviation 
 $\delta_{l2}-B\Delta\delta_1$, where $B$ is an integer and scan outward again 
 with step size $\Delta\delta_1/D$.  This procedure is called a ``step-back/split'' and can be
...
\item N.B.: This command is {\em not recommended}.  It is {\em far} better to put constraints
...
If the twiss command was given  with \verb|output_at_each_step=1|, one may also refer to statistics of
...
If the twiss command was given  with \verb|output_at_each_step=1| and radiation integral computation
...
The coupling integral and emittance ratio due to x-y coupling may
...
\item {\tt dnux/dAx}, {\tt dnux/dAy}, {\tt dnuy/dAx}, {\tt dnuy/dAy} --- Tune shifts with amplitude.  These will
...
\item {\tt alphac}, {\tt alphac2} --- First- and second-order momentum compaction.  The path length is 
  $s = s_o + \alpha_c\delta + \alpha_{c2}\delta^2$.
...
\item {\tt sddsbrightness} --- Uses twiss parameter output or data from {\tt sddsanalyzebeam} to
  compute undulator brightness curves.  
...
evolution of the emittances.  The IBS algorithm is based on an
extension of the ZIBS routine in the program ZAP (Zisman, {\em et
al.}).

\item {\bf examples:}
This example computes the IBS equilibrium parameters and the contributions to the
growth rates vs position in the APS lattice.
\begin{flushleft}{\tt 
...
{-charge={\em nC}|-particles={\em value}} -coupling={\em value}
[-emitxInput={\em value}] [-deltaInput={\em value}] 
[-growthRatesOnly] [-superperiods={\em value}]
{-RF=Voltage={\em MV},harmonic={\em value}|-length={\em mm}}
[-energy={\em MeV}] [-integrate=turns={\em number}[,stepSize={\em number}]]
}\end{flushleft}
...
\item {\bf -growthRatesOnly} --- If given, only the growth rates are computed.  Equilibrium
...
\item -integrate=turns={\em number}[,stepSize={\em number}] --- If given, then {\em resultsFile}
 contains the result of integrating the differential equations for the emittances for 
 the given number of turns.  The step size is the number of turns for each integration step,
 and can be adjusted to get faster results.
\end{itemize}

\item {\bf author:} L. Emery, M. Borland, ANL/APS.
\end{itemize}
...
\item {\tt correctedOnly} --- If given, only the corrected twiss parameters are computed and
 output.  These parameters are the correct ones to match a beamline to, since they have the
 dispersive and mono-energetic terms properly separated.
\end{itemize}
...
distribution of {\tt elegant}.  The examples are intended to
...
\end{verbatim}
Similarly, to find all examples that use the \verb|CSBEND| elements, one could use
\begin{verbatim}
...
\bibitem{Ripken}
 G. Ripken, DESY Report Nno. R1-70/04, 1970 (unpublished).
