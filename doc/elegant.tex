...
\author{Program Version 14.9.1\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 14.9.1}

Here is a summary of what's changed since version 14.8.  For details
and references, see the specific entries in the manual.

As many of you know, I was on leave of absence from ANL for one year,
and worked at Lyncean Technologies, Inc.  LTI generously agreed that
all the changes I made to {\tt elegant} and related programs while
there would be shared with the accelerator physics community.
...
\begin{itemize}
\item Added LSCDRIFT, which provides simulation of longitudinal space charge
  in a drift space, based on an impedance provided by Z. Huang (SLAC). 
\item Added FRFMODE and FTRFMODE elements. These are similar to RFMODE and
  TRFMODE, in that they simulate cavity modes.  However, FRFMODE and
  FTRFMODE take the mode data from a file.  If there are several modes
  to be simulated, this is {\em much} faster than using several
  RFMODE or TRFMODE elements.
\item Added TFBPICKUP and TFBDRIVER, a pair of elements for simulation of
  single-bunch transverse digital feedback systems.
\item Added FLOOR element for setting floor coordinates inside a beamline.
  This can be used, for example, to optimize the initial coordinate and
  direction of a beamline so that it terminates at a required location and
  direction.
\item Added LTHINLENS and LMIRROR elements, which provide modeling of 
  thin lenses and mirrors for light beams.
\item Added EMATRIX element, allowing explicit matrix input on the element
  definition line (instead of in a file as in MATR element).  This was suggested
  by P. Emma (SLAC).
\end{itemize}
...
\item Fixed the units for K3 and K4 parameters on CSBEND and CSRCSBEND.

\item Fixed a bug in the low-pass filter function for CSRCSBEND, which may have resulted
 in noise in the highest frequency bin.  (I haven't seen any effect on simulation results.)

\item Fixed a bug in the FMULT (file multipole) element
...
\item Fixed dispersion and path-length terms in the corrector matrix for HKICK, 
...
\item Fixed problem with the MATTER elements that occurred with length
 was exactly a radiation length (there was no scattering!).  This bug was 
 discovered by P. Emma (SLAC).

\item The BNL parameter of the MULT element has been renamed BTIPL, which is a more
 accurate description.

\item RFTMEZ0 (TM-mode rf cavity from on-axis longitudinal field):'
 \begin{itemize}
 \item Now allows independent misalignment of cavity and solenoid fields.
 \item Added stray magentic fields.
 \item Is now active during trajectory correction.
 \end{itemize}

\item Twiss parameter computation is now correct when the ROTATE
element is used with an angle of 90 or 270 degrees.

\item The SREFFECTS element (which simulates lumped quantum excitation
 and radiation damping in storage rings), now works correctly when $\alpha_x$ or 
 $\alpha_y$ are nonzero.

\item The STRAY element (stray fields) now works for tilted beamlines.  The matrix
...
\subsection{Improvements to Elements}
\begin{itemize}
\item Added GROUP parameter to all elements.  It allows assigning a common tag to
 elements.  The tags are saved in the lattice and parameter files.

\item The B2 (sextupole) parameter was added to the BUMPER element (which does
 time-dependent kicker simulation).  Also, the N\_KICKS parameter was added
 and must be non-zero if B2 is to have any effect.

\item The CSRCSBEND now has a wake filter file capability, allowing the user to 
 specify a filter function as a way to add, for example, shielding effects.

\item The HISTOGRAM and WATCH (coordinate mode) now input parameter ``s'' giving the position
...
\item Added steering capability to KQUAD and QUAD using an imposed dipole field,
...
\item The MODRF (modulated rf cavity) now supports damping rates for AM and PM 
 modulations.

\item Several improvements were made to the NIBEND (Numerically Integrated BEND) element:
 \begin{itemize}
 \item Added new fringe models: enge1, enge3, and enge5.  These
 give 1st, 3rd, and 5th order expansions of three-coefficient Enge functions.
 \item Added FP2 through FP4 parameters for NIBEND.  They are used for
 direct input of Enge coefficients.  
 \item Added ADJUST\_FIELD parameter, which provides a way to adjust
 the deflection angle to compensate for over- or under-bending due to fringe fields.
 \item Added ADJUST\_BOUNDARY parameter to allow turning off boundary position adjustment,
 which is the default method for adjusting the deflecting angle.
 \item Misalignments are now supported.
 \end{itemize}

\item Matrices for the QUAD and SEXT elements may now have order up to 3.  This
 can make a difference in momentum-dependent tune shifts and nonlinear dispersion
 computations.  

\item The RFCW element was changed extensively:
 \begin{itemize}
 \item The N\_KICKS parameter can now have a value other than 0 or 1.  A non-zero value
 results in a drfit-kick-drift simulation with N\_KICKS segments.  
 \item By default, the longitudinal and tranverse wakes are now applied at the location of
 the rf kick, which is a change from previous versions.  Use the WAKES\_AT\_END parameter
 (set to 1) to revert to the previous behavior.
 \item Added LSC parameter to the RFCW element, to provide
 simulation of longitudinal space charge in an accelerating cavity.
 \end{itemize}

\item Added FREQ\_WAVEFORM and Q\_WAVEFORM support to the RFMODE element, allowing
...
\item The SCRIPT element has new USE\_CSH and VERBOSITY controls.  Setting
 USE\_CSH=0 is much faster and should be used if C-shell is not needed. 

\item TRWAKE: 
 \begin{itemize} 
 \item Added XFACTOR and YFACTOR parameters (which default to 1).  These allow multiplying
 the wakes by factors.
 \item Added TILT parameter.
 \item Added XPOWER and YPOWER fields, which allows simulating transverse wakes
 due to asymmetric chambers (by setting XPOWER=0 and/or YPOWER=0).  By default, 
 XPOWER=1 and YPOWER=1.
 \end{itemize}

\item The ZLONGIT and ZTRANSVERSE elements now check that the input data is equispaced
 in frequency.  This bug was found by T. Shaftan (BNL).
\item Added FACTOR and REVERSE\_TIME\_ORDER parameters to ZLONGIT.  The latter allows
 compensating for different sign conventions for the meaning of the longitudinal coordinate.
 (Recall that in {\tt elegant}, a negative longitudinal coordinate indicates the head of the
 bunch.)

\end{itemize}

\subsection{Modified Commands and Bug Fixes to Commands}
\begin{itemize}
 \item {\tt amplification\_factors}
 \begin{itemize}
 \item Now sets all correctors to zero before computing a new amplification factor.
 \item Output now includes kick amplification function for each
 actuator as well as useful parameters in all the output files.
 \item Added amplification factor output for slopes.
 \end{itemize}

\item {\tt bunched\_beam}: Previous versions did not generate
longitudinal phase space for dynamic aperture beam type.  This version
does.

\item Several improvements where made to orbit correction and closed orbit
 computation:
 \begin{itemize}
 \item  The orbit correction matrix is now computing using the average
 Twiss parameters at the entrance and exit of the elements.  The previous
 version used just the exit parameters.
 \item BPM-weighted correction works properly now.  In past versions, the
 weight was basically either 0 (exclude) or 1 (include).
 \item Added output of the orbit slopes to the closed orbit output file.
 \item The fixed-path-length orbit computation algorithm now has improved
 convergence.
 \item Pre-correction corrector zeroing now includes objects other than HKICK, VKICK,
 and KICKER.
 \end{itemize}

\item {\tt error\_element}: 
 \begin{itemize} 
 \item Added {\tt bind\_across\_names} parameter, which permits
 ...
 \item Previously, the "plus\_or\_minus" error type was also necessarily and
...
\item {\tt find\_aperture}: Added {\tt search\_output} parameter, which gives a filename
 to which details on each tracked particle will be written.

\item {\tt load\_parameters}
 \begin{itemize}
 \item Now correctly handles loading and saving parameters when element division is used.
 \end{itemize}

\item Optimization
 \begin{itemize}
 \item Added two new optimization methods: ``randomwalk'' and ``randomsample''.
 These are very useful when starting from scratch with a system that is, for example,
 unstable.
 \item Added {\tt bestOptimizationFunction} parameter to final output file.  This 
 gives the best value of the optimization function obtained so far.
 \item Chromaticity correction is now optionally performed while doing an optimization.
 (One can also include the chromaticity in the optimization literally, of course.)
 \item Added the ability to automatically change the weights of different optimization
 function terms to emphasize the most difficult terms.  Paradoxically, this can often
 result in convergence for all terms.  
 \item Added controls for adjustment of initial simplex size on restart of simplex
 optimization. 
 \item Higher-order dispersion and chromaticities are now available for optimization expressions.
 \end{itemize}

\item {\tt sdds\_beam}:
 \begin{itemize} 
 \item Fixed bug in the code that checks units for elegant-style input
files.  In previous versions, if the units of p were blank (an allowed
``error''), other units errors were ignored.
 \item Now read $p_\phi$ data for {\tt spiffe} input files. ({\tt spiffe} is
a particle-in-cell code for rf gun simulation, available from the OAG web site.)
 \end{itemize}

\item {\tt track}: Fixed bug that prevented linear chromatic matrix tracking (LCMT) from
...
\item {\tt tune\_shift\_with\_amplitude}:
 \begin{itemize}
 \item Added controls for adjustment of the initial particle amplitude.
 \item Fixed problems with computation in presence of MALIGN elements with ON\_PASS parameters
 not equal to -1.
 \end{itemize}
...
\subsection{Changes to Related Programs}

\begin{itemize}
\item The program {\tt ibsEmittance} now provides IBS scattering rate contributions
  as a function of position in the ring.
\item The program {\tt madto} now attempts to output COSY format.  Users should
 check the results carefully!
\item The program {\tt sddsanalyzebeam} now copies input parameters through to output columns.
 It also has a new option, {\tt -correctedOnly}, which results in computation of {\em only} 
 the dispersion-corrected twiss parameters. 
\item Added the program {\tt sddsrandmult}, which computes random error multipole
 levels for specified quadrupole and sextupole construction errors.  The output
 of {\tt sddsrandmult} is accepted by the KQUAD and KSEXT elements.
\end{itemize}
...
\item {\tt elegant} now builds on the IBM AIX platform, but has not 
been tested.  This was done by R. Soliday (ANL).

\item Added support for checkpointing using the CONDOR checkpointing
library.  Contact the author if you need to use this feature (Linux 
only, gcc 2.9X).
...
again using step sizes given determined from the range of the simplex
and the factor \verb|simplex_pass_factor|.
...
\item {\tt spiffe}: ${\rm (r, z, pr, pz, pphi, t)}$, where r and z are
  ...
  ${\rm p_\phi = \omega r \gamma/c}$,  and t is in seconds.
\end{itemize}
