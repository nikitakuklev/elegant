...
&momentum_aperture
    ...
    double x_initial = 0;
    double y_initial = 0;
    double delta_negative_start = 0.00;
    double delta_negative_limit = -0.06;
    double delta_positive_start = 0.00;
...
Limits of scans in the negative and positive directions.
\item \verb|delta_negative_start|, \verb|delta_positive_start| ---
Start values of scans in the negative and positive
directions. Scanning begins at these and moves toward the respective limits.
\item \verb|delta_points| --- Initial number of values of $\delta$ to
 use.  This is used to determine the initial step size.  It is
 important to make this value large enough that one doesn't skip over
 a narrow region where particles get lost.
...
The idea for this command is based on M. Belgrounne {\em et al.},
``Refined Tracking Procedure for the SOLEIL Energy Acceptance
Calculation,'' Proceedings of PAC 2003, p 896.  In particular, the
energy aperture as a function of position around the ring is
determined by tracking.  Starting at the beginning of the lattice and
working downstream, particles are tracked starting from the exit of
each selected element.  The betatron coordinates are initially zero
(or very small), while the momentum deviation is gradually increased
until loss of the particle is observed.  This defines the momentum
aperture at that location.

The user must be careful when using this command not to make
excessively large steps, unless the back-stepping and splitting
feature is employed.  When tracking in 6 D, the momentum aperture
boundary may be chaotic, meaning that the location of the boundary is
ill-defined.  It is suggested that an initial run be used to determine
the boundary approximately.  Following this, the starting values may
be adjusted to allow a finer scan to be performed without excessive
computational time.
