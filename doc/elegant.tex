...
\author{Program Version 33.2\\Advanced Photon Source\\Michael Borland, Tim Berenc\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 33.2}
...
\item \verb|log_file| --- The (incomplete) name of an SDDS file to which statistical data will be written as the simulations
  ...
  slightly. 
\item \verb|delta_min| --- Minimum absolute value of $\delta$ from scattering.  Should be small enough that no particle scattered
  by this $\pm$ this value is lost, regardless of the scattering location. 
\item \verb|delta_max| --- Maximum absolute value of $\delta$ from scattering. Should be large enough that no particle scattered
  by $\pm$ this value survives, regardless of scattering location. 
\item \verb|n_delta| --- Number of $\delta$ scattering angle values on the range \verb|delta_min| to \verb|delta_max|.
  The same number will be scattered in the range -\verb|delta_min| to -\verb|delta_max|.
\item \verb|momentum_aperture|, \verb|momentum_aperture_scale| --- If given, names a file giving the momentum aperture vs s, which
  ...
  Such a file may be obtained from running the \verb|momentum_aperture| command. The \verb|deltaNegative| column will be 
  used in place of \verb|delta_min| for negative-side scatters, while the \verb|deltaPositive| column will be used for
  used in place of \verb|delta_min| for positive-side scatters.
  The \verb|delta_min| values thus obtained are multiplied by \verb|momentum_aperture_scale|, so there is some assurance
...
\item \verb|field_calculation_method| --- Provided for future expansion. At present, the fields are computed on the
  ...
  for the ions.
\item \verb|macro_ions| --- The number of macro ions to generate per bunch per turn. The macro ion charge is adjusted
