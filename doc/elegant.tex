...
\author{Program Version 28.1.0\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 28.1.0}
...
\begin{itemize}
\item Fixed bug that resulted in inconsistent fiducialization across processors when the beam was nonuniform across processors
  and the \verb|start_from_centroid| flag was given to the \verb|correct| command.
\item Now issue warnings if \verb|CSBEND| must be replaced by a \verb|EDRIFT| or \verb|KQUAD| because $\rho>10^6$m.
\end{itemize}
...
Contributors to {\tt elegant} include M. Borland, M. Carla', N. Carmignani, M. Ehrlichman, W. Guo, R. Soliday, V. Sajaev, C. Wang, Y. Wang, Y. Wu, and A. Xiao.
...
\begin{equation}
B(\omega) = \frac{1}{N}\sqrt{\sum_{i=1}^N (\cos^2 \omega t_i + \sin^2\omega t_i)},
\end{equation}
