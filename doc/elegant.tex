...
\author{Program Version 35.1.0\\Advanced Photon Source\\Michael Borland, Tim Berenc\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 35.1.0}
...
\begin{itemize}
\item The \verb|MKPOLY| element was added. It allows imposing multiple polynomial kicks up to 3$^{rd}$ 
  order on the beam in either the x or y plane.
\end{itemize}
...
\item \verb|share_tracking_based_matrices| --- If non-zero, then the matrices determined by tracking for various elements
  (e.g., \verb|BRAT|, \verb|BGGEXP|, \verb|CCBEND|) are computed only once for a set of identical elements, then shared.
\item \verb|tracking_matrix_step_factor| --- The default step size for tracking-based matrices is $10^{-5}$ (in the appropriate units
  for each corodinate). This can be increased or decreased by supplying a value for \verb|tracking_matrix_step_factor|.
