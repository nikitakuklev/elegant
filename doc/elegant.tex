...
\author{Program Version 15.4.1\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 15.4.1}

Here is a summary of what's changed since release 15.3.  For details
...
\begin{itemize}
\item Fixed bug in synchrotron radiation simulation for {\tt KQUAD} and {\tt KSEXT}
 when the (default) 4$^{th}$-order integrator was used.  This would have impacted
 off-momentum tracking simulations with synchrotron radiation.  The error amounted
 to applying only half the required energy loss.  Note that tracking with energy
 oscillations was not really affected, since in this case there is no average effect
 from the quadrupoles or sextupoles.  Also, gradient dipole simulation was not affected.

\item The {\tt SREFFECTS} element (synchrotron radiation effects) will now mark
 all particles as lost in the event that the computed energy loss per turn is positive.
 This odd condition can happen, for example, when the closed orbit iteration
 doesn't converge, usually because the lattice is unstable.
 In previous versions, {\tt elegant} would simply exit.  This bug was reported by
 V. Sajaev (ANL).

\item Fixed bug in the lattice input parser, which resulted in embedded {\tt rpn} commands
 being ignored if they directly followed a comment.  (This was reported by a user but
 I've lost track of who.  Sorry about that.)
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item Fixed a bug in the {\tt matrix\_output} command for the SDDS output option.
 In the past, no data was provided at the exit of elements (e.g., markers, BPMs)
 that did not have a non-trivial matrix.  This bug was reported by A. Xiao (FNAL).

\item Improved chromaticity calculations from {\tt twiss\_output} so that 
 chromaticity and the chromatic derivative of the beta function for transport lines
 is now correct.   The previous code assumed a periodic solution.  This deficiency
 was reported by V. Sajaev (ANL).  Users may notice very small changes in chromaticity
 calculations for rings due to reorganization of some expressions. Also, if higher-order
 chromaticity is requested by a periodic (``matched'') solution is not, {\tt elegant}
 will abort.

\item The {\tt tune\_shift\_with\_amplitude} command no longer aborts if the ratios
 of the minimum and maximum amplitudes are inadequate for tune {\em shift} calculation
 if only tune {\em spreads} are requested.

\item Fixed bug in {\tt find\_aperture} command that resulted in varied parameter values
 not being set in the output files.

\item Fixed several bugs in radiation integral calculations:
 \begin{itemize}
 \item The effect of steering kicks from quadrupoles on the radiation
 integrals are now included.  (This is not the same as the effect of misalignments,
 which was already there.)
 \item An adjustment factor that modified the focusing and angle of bending
 magnets in the radiation integral calculation according to the momentum deviation
 was removed.  It was found to cause greater disagreement with tracking results from
 element-by-element simulation for large momentum deviations.
 \end{itemize}

\end{itemize}

\subsection{Modified Elements}
\begin{itemize}

\item Added noise-group feature to the {\tt RFTM110} element to allow imparting correlated
 phase and voltage noise to multiple cavities.  Also added voltage ramping option using
 a user-specified table.

\item Added quantum excitation simulation for the {\tt KQUAD} and {\tt KSEXT} elements.

\item Improved quantum excitation simulation for the {\tt CSBEND} and {\tt CSRCSBEND} elements
 by including the variation of quantum excitation with path-length, local field, and individual
 particle momentum.  Note that this is a small effect.  The variation of the classical component
 of the radiation with these factors was always there, and is what determines the damping
 partition factors.

\item Added the {\tt DISABLE} parameter for {\tt WATCH} elements.  Using this is easier than,
 for example, using the {\tt transmute\_elements} command to turn them into zero-length drift spaces.
\end{itemize}

\subsection{Modified Commands}
\begin{itemize}
\item The {\tt find\_aperture} command now includes the method of searching along one or more
 lines starting at the origin.  In previous versions, the search always started from the outside,
 which is faster but may be misleading due to islands.  This was suggested by V. Sajaev (ANL).
 The ability to offset the aperture region with the closed orbit was also added.
\item The {\tt print\_dictionary} command will now output data to an SDDS file.  This is useful
 if for some reason you need to automatically process the element dictionary.
\end{itemize}

\subsection{Changes and Additions to Related Programs}
\begin{itemize}
\item The program {\tt sddsbrightness} now optionally uses data produced by {\tt sddsanalyzebeam}.
...
\item The GUI front-end {\tt elegantRingAnalysis} had several upgrades:
 \begin{itemize}
 \item Added frequency maps in $(\delta, x)$ and $(\delta, y)$ planes.  This was suggested by
 L. Nadolski (SOLEIL)
 \item Added color-coding of phase space plots with the tune.
 \item Added more controls on plotting for FMA and fine dynamic aperture.
 \item Added optional $\delta$ offset for phase space tracking.
 \item The default matrix order for all tracking using the kick lattice is now 1.  That means that
 ...
 with an additional parameter file or by editing the lattice file to explicitly add {\tt ORDER}
 values to the elements in question.
 \end{itemize}
\item Quantum lifetime calculations from {\tt beamLifetimeCalc} now include the transverse
...
\subsection{Other Changes to This Document} 
\begin{itemize}
\item Added manual page for {\tt haissinski} program.
\item Added manual page for {\tt sddsbrightness} program.
\item Improved discussion of simplex optimizer controls for {\tt optimization\_setup} command.
\item Added warning about using the {\tt CHANGE\_T} feature of {\tt RFCA} in the case
 of multiple cavities that are not harmonically-related.
\end{itemize}
...
occurs if \verb|bind| is nonzero; if \verb|bind| is negative, then the
...
\item \verb|x_initial|, \verb|y_initial| --- The initial x and y coordinate values for tracking. It is particularly
 important that \verb|y_initial| be nonzero if one wants to see losses due to resonances.
\item \verb|delta_negative_limit|, \verb|delta_positive_limit| --- Limits of scans in the negative and positive direction.
  By default, scanning begins at zero and moves toward these limits.  
\item \verb|delta_points| --- Initial number of values of $\delta$ to use.  This is used to determine the initial step size.
 It is important to make this value large enough that one doesn't skip over a narrow region where particles get lost. 
\item \verb|n_splits| --- Number of times to split the step size in order to refine the location of the maximum surviving
 momentum offsets.  One additional particle will be tracked for each split made.
\item \verb|scan_from_outside| --- By default, scanning begins from 0 and moves outward.   Setting
 this parameter to 1 results in a scan from the outside toward zero, which is faster but potentially unreliable,
 in that the results may be affected by stable islands.
\item \verb|s_start|, \verb|s_end| --- Limiting s coordinates of the elements from which tracking will start.  The
 default values will exclude no elements.
\item \verb|include_name_pattern| --- If given, tracking will start only at the entrance to elements that match the given wildcard pattern.
\item \verb|verbosity| --- Larger values result in more detailed printouts as calculations proceed.  Mostly for debugging.
\end{itemize}

The algorithm for this command is based on M. Belgrounne {\em et al.}, ``Refined Tracking Procedure for the SOLEIL Energy Acceptance Calculation,''
Proceedings of PAC 2003, p 896.   In particular, the energy aperture as a function of position around the ring is determined by tracking.
Starting at the beginning of the lattice and working downstream, particles are tracked from the entrance of each element.  The betatron
coordinates are initially zero (or very small), while the momentum deviation is gradually increased until loss of the particle is observed.
This defines the momentum aperture at that location.

N.B.: The present version of the code does not center the particles on the closed orbit!  This will be added in the next version.
...
given, a beam-size or centroid name, a sigma-matrix element, or a
closed orbit value ({\tt xco}, {\tt xpco}, etc) if closed orbit
calculations are requested.  The parameter names are the same as those
used in the corresponding output files.  Beam sizes, centroids, and
sigma-matrix elements are from tracking the particle distribution.
See the documentation for the MARK element for a more detailed listing.

If closed orbit calculations are requested, data from {\tt MONI}, {\tt
HMON}, and {\tt VMON} elements that have {\tt CO\_FITPOINT=1} is
available under the names {\em bpmName}\verb|#|{\em occurence}.{\em q}{\tt
co}, where {\em q} is {\tt x}, {\tt xp}, {\tt y}, or {\tt yp}.
...
\item {\em V1} {\em V2} {\em T} {\tt segt}.  Returns a
 nonzero value if and only if value {\em V2} is greater than {\em V1}.  The 
...
\item {\em V1} {\em V2} {\em T} {\tt selt}.  Returns a
 nonzero value if and only if value {\em V2} is less than value {\em V1}.  The 
 returned value is $((V_1-V_2)/T)^2$.  
