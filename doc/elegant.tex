...
\author{Program Version 22.0\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 22.0}

Here is a summary of what's changed since release 21.0.  For details and references, see the specific entries in the manual. 

There have been relatively few changes to the serial code, apart from some mostly minor bug fixes.
The major change in this version is a reworking of parallelization to include parallel I/O, parallel frequency map analysis (\verb|frequency_map|), and parallel 
momentum aperture determination (\verb|momentum_aperture|).  This work was done by Y. Wang and H. Shang.

\subsection{Bug Fixes for Elements}

\begin{itemize}
\item Fixed some problems with keeping track of whether radiation integrals have been computed, which prevents invalid \verb|SREFFECTS|
setup in some cases.  
\item Fixed a bug in computing the radiation matrix of a tilted \verb|SBEND| element, which would affect beam moments analysis.
\item Since the matrix for the \verb|UKICKMAP| element is determined by tracking, the \verb|N_KICKS| parameter may in principle change
 the matrix.  This fact was not previously part of the element definition, but has been fixed.
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item Fixed a bug in trajectory/orbit correction that resulted in a crash when the number of monitors or correctors was zero.
\item In frequency map analysis, a fiducial particle is now tracked first to ensure that the system is properly fiducialized.  This is needed
if there is an rf cavity in the lattice (which isn't really a good idea for FMA).
\end{itemize}

\subsection{New and Modified Elements}
\begin{itemize}
\item Added \verb|INCLUDE_OFFSETS| parameter to \verb|SREFFECTS|, which allows the user to control whether dispersion-related
offsets of the orbit are included when a particle loses energy.  To avoid unwanted quantum excitation, we no longer
include offsets (i.e., dispersion effects) when tracking with only radiation losses.
\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item Added the ability to optimize dynamic aperture using \verb|find_aperture|.
\item Optimization terms may now be defined using an external SDDS file, which can make it easier to set up 
 optimizations with large numbers of terms.
\end{itemize}

\subsection{Changes to Related Programs and Files}

\begin{itemize}
\item A new program, {\tt sddsfluxcurve}, was added.  It computes flux tuning curves for undulators using
beam parameters from {\tt elegant}'s \verb|twiss_output| command or from {\tt sddsanalyzebeam}.
\end{itemize}
