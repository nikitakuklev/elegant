...
Here is a summary of what's changed since release 22.0.  For details and references, see the specific entries in the manual. 
...
\item The script {\tt astra2elegant} was replaced by a compiled program.  The program is not only much faster, but
  also has corrections for the conversion that will be evident in long bunches or low energy beams.
\end{itemize}
...
on subsequent pages.
\begin{itemize}
\item {\tt astra2elegant} --- Converts ASCII particle output from ASTRA to a binary
...
\item {\bf synopsis:}
\begin{flushleft}{\tt
astra2elegant [{\em inputFile}] [{\em outputFile}] [-pipe=[input][,output]]
}\end{flushleft}
