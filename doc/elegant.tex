...
\item The matrix for the \verb|SBEN| element was incorrect when the length was negative (which is needed for
  back-tracking), as pointed out by Y. Park (UCLA). A. Zholents (ANL) and Y. Park (UCLA) helped identify the problem.
\end{itemize}
...
\item The \verb|ion_effects| command was improved in several ways: 
  ...
  \item The default distribution fitting criterion is the sum of the rms deviation and the absolute fractional
    deviation of the charge. This makes it less likely that overfitting will result in large spikes in the
    distribution.
  \item The new \verb|ion_histogram_max_bins| parameters allows restricting the maximum number of bins; it defaults to 100,000.
  \item The new \verb|ion_histogram_min_per_bin| parameter allows setting a requirement on the minimum number of 
    macro ions per bin; it defaults to 15.
...
\iterm \verb|pressure_factor| --- Factor by which to multiply the pressures given in the \verb|pressure_profile|.
...
\item \verb|fit_residual_type| --- Residual type for distribution fitting. The default is \verb|rms-dev-plus-ad-charge|, which
  indicates using the sum of the normalized rms deviation and the normalized absolute deviation of the total charge, where
...
\item \verb|back_tracking| --- If nonzero, then back-tracking is performed. The beamline is reversed in
  order and the beam is propagated backwards through the elements. Only a few elements are supported at
  present, including \verb|CHARGE|, \verb|DRIF|, \verb|HMON|, \verb|MARK|, \verb|MONI|, \verb|QUAD|, \verb|RFCA|, 
  \verb|SBEN|, \verb|SEXT|, \verb|TRWAKE|, \verb|VMON|, \verb|WAKE|, and \verb|WATCH|.
\end{itemize}
