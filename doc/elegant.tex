...
\author{Program Version 2019.2.0\\Advanced Photon Source\\Michael Borland, Tim Berenc\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 2019.2.0}
...
\item The \verb|ion_effects| command now supports calculation of ion fields using a bi-gaussian distribution (sum of two
  gaussians), which allows modeling the core and tails of the distribution more accurately. A number of parameters
  were added for control of fitting and output. J. Calvey (ANL) co-developed this improvement.
\end{itemize}
...
&ion_effects
	  ...
	  long bigaussian_fit_evaluations = 20;
	  long bigaussian_fit_passes = 2;
	  long bigaussian_fit_restarts = 20;
...
\item \verb|field_calculation_method| --- By default, the fields are computed on the
  ...
  for the ions. A more accurate, but slower, method is a sum of gaussians, which can be invoked by setting
  \verb|field_calculation_method| to \verb|"bigaussian"|.
\item \verb|bigaussian_fit_target| --- If the bigaussian field calculation method is selected, gives the target for the
  fractional deviation of the fit. Smaller numbers will result in long run times.
\item \verb|bigaussian_fit_tolerance| --- If the bigaussian field calculation method is selected, gives the tolerance for the
...
\item \verb|bigaussian_fit_evaluations|, \verb|bigaussian_fit_passes|, \verb|bigaussian_fit_restarts| --- Parameters
  for the simplex optimizer that performs the bigaussian fit. Note that in \verb|Pelegant|, a hybrid simplex method is
  used, which appears to converge quickly if the default parameters are used.
\item \verb|fit_residual_type| --- Residual type for bigaussian fitting. The default is \verb|rms-dev-plus-ad-sum|, which
  indicates using the sum of the normalized rms deviation and the normalized absolute deviation of the total charge.
  ...
  deviation and normalized rms deviation), \verb|sum-ad-plus-rms-dev|, \verb|rms-dev-plus-ad-sum|, and \verb|sum-ad-plus-ad-sum|.
...
\item \verb|ion_bin_divisor| --- For \verb|field_calculation_method="bigaussian"|, the number of ion bins per rms parameter of
      the electron beam.      
\item \verb|ion_range_multiplier| --- For \verb|field_calculation_method="bigaussian"|, the full span of the ion binning regoin
      bins in units of the rms parameter of the ion distribution.
\item \verb|ion_sigma_limit_multiplier| --- For \verb|field_calculation_method="bigaussian"|, the minimum value for either of the
  ion sigmas in units of the bin size. Use to prevent one of the gaussians from being too delta-function-like.
\item \verb|ion_histogram_output|, \verb|ion_histogram_output_s_start|, \verb|ion_histogram_output_s_end|, 
      ...
      \verb|field_calculation_method="bigaussian"|. \verb|ion_histogram_output| gives the (incomplete) filename.
      ...
      \verb|ion_histogram_min_bins| gives the minimum number of bins worth of data to output; if the number exceeds this,
      output is restricted to the most populous part of the histogram 
