...
\author{Program Version 17.1.1\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle

\section{Highlights of What's New in Version 17.1.1}

This release is a fairly major one, particularly with respect to the
degree of parallelization of collective effects, thanks to on-going
work by Yusong Wang.  Parallelization now includes wakefields and
coherent synchrotron radiation, in addition to the single-particle
dynamics that was parallelized in the previous release.  Elements that
support parallel computation are indicated on the individual manual pages.
Just under 80\% of the elements have been parallelized at this time.
Elements that are not yet parallelized can still be used as part of a
parallel run, but the computations for those elements are done in serial mode.

In addition, several serious bugs were found and fixed, and numerous
improvements were made.  Here is a summary of what's changed since
release 16.1.7.  For details and references, see the specific entries
in the manual.
...
\subsection{Bugs Fixes for Commands}
\begin{itemize}
\item Fixed bug in {\tt load\_parameters} that resulted in slow loading of data when a
 specific occurrence of an element was not found.
\item Fixed bug in lost-particle saving (via the {\tt run\_setup} command), which was
 introduced in version 16.1.7.  The bug resulted in a crash of the program.
\end{itemize}

\subsection{Bug Fixes for Elements}
\begin{itemize}
\item A lattice file parser bug was fixed.  Previously, elements with
  the same name as a beamline and beamlines with the same name as an
  element were allowed, resulting in incorrect saved files in some
  cases.  The program now issues an error message and exits when
  duplicate definitions are encountered.
\item Fixed problems with single-pass mode of the {\tt RFMODE}, {\tt TRFMODE},
  {\tt FRFMODE}, and {\tt FTRFMODE} elements.  Previous versions used an uninitialized
  variable and may give unreliable results depending on the compiler.
\item The {\tt TRFMODE} element now includes the longitudinal voltage (a very small effect in
 most cases).
\end{itemize}

\subsection{New and Modified Elements}
\begin{itemize}
\item The {\tt RFDF} element (RF deflector) was upgraded to include the longitudinal
 field and to treat standing-wave deflectors.  It also now
 supports the same modulation and noise features as {\tt RFTM110}.
\item The {\tt RFMODE}, {\tt TRFMODE}, {\tt FRFMODE}, and {\tt FTRFMODE} now support ramping 
 of the impedance, which helps to reduce transient effects.  This was requested by Y. Chae (ANL)
 based on ideas from M. Blaskiewicz (BNL).
\item Added ``binless'' mode for {\tt RFMODE} and {\tt TRFMODE}, which is much faster than 
 the histogram-based method for beams with trains of bunches.
\item Added distribution-based synchrotron radiation to the {\tt CSBEND} element.  This provides
 a more fundamental simulation of quantum excitation, in that accurate photon energy, number,
 and angular distributions are used.  (Even so, little difference is seen from the previous 
 model based on guassian distributions.)
\item Added the {\tt RECORD} and {\tt SAMPLE\_INTERVAL} features to {\tt TRFMODE}, allowing
 output of the mode voltage and other data.
\item Added the {\tt MAX\_BINS} feature to {\tt ZLONGIT} and {\tt ZTRANSVERSE}, to prevent
 use of an excessive number of bins.
\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item The {\tt rpn\_load} command was added, which helps in multi-stage optimization by allowing
 loading and selection of data from SDDS files into the RPN variables that can be used in
 optimization expressions.
\item The {\tt optimization\_setup} command now allows specifying that the random number generator
 shall be reset to the user-specified seed at the start of every function evaluation. In addition, initial
 values of variable and covariable parameters are now available in RPN variables for use in expressions.
\item The {\tt load\_parameters} commands now limits the number of warnings about missing parameters
 to 100, after which warnings are suppressed.
\item The {\tt momentum\_aperture} command now supports fiducialization of the beamline momentum profile
 before determining the momentum aperture.  This is useful when there is acceleration or energy loss
 due to radiation.
\end{itemize}
...
\begin{itemize}
\item {\tt sddsurgent}: This program was in the previous release, but was undocumented.  It uses algorithms from the programs 
...
\item {\tt beamLifetimeCalc}: When solving the Haissinski equation for the bunch length, the momentum 
 compaction entered into the GUI now overrides the value from the lattice file.
\item {\tt elegantRingAnalysis}:  Errors are now implemented in a global fashion that may be applied to many calculation types.
\item {\tt touscheckLifetime}: This new program (by A. Xiao) computes the Touschek lifetime using Piwinski's formalism.
 It takes twiss parameter and s-dependent momentum-aperture data from {\tt elegant}.
\end{itemize}

\section{Credits}

Contributors to {\tt elegant} include M. Borland, V. Sajaev, Y. Wang, Y.Wu, and A. Xiao.
...
\begin{itemize}
\item type: setup command.
\item function: set up for transverse space charge calculation (ring only).
\end{itemize}
...
Data from MARK elements with FITPOINT=1 may be used via
symbols of the form
