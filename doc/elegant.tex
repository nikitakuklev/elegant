...
\author{Program Version 26.0.2\\Advanced Photon Source\\Michael Borland\\ \date{\today}}
\maketitle
...
\section{Highlights of What's New in Version 26.0.2}

Here is a summary of what's changed since release 25.3.2:

\subsection{Bug Fixes for Elements}

\begin{itemize}
\item Corrected the second- and third-order matrices for the \verb|KQUSE| element. These matrices were used for analysis (e.g., chromaticity) and 
  tracking when \verb|MATRIX_TRACKING=1| was set as part of the element definition. This bug was reported by C. Tennant (TJNAF).
\item Corrected the third-order matrix for \verb|QUAD| and \verb|KQUAD| elements, and certain (off-diagonal) elements of the third-order matrix
  for the \verb|OCT| and \verb|KOCT| elements. All of these corrections were traced to a bug in a mathematica script that computes the matrices.
\item The \verb|BUMPER| element now uses the kicker waveform when a kickmap is supplied. Previously, the kicker waveform got ignored when a 
  kickmap was provided. Also, the \verb|BUMPER| element now behaves as expected when the length is zero.
\item The steering feature of the \verb|KQUAD| element did not work properly with the \verb|moments_output| command.
  This bug was reported by V. Sajaev (APS).
\item The steering multipoles feature of \verb|KQUAD| did not correctly include the skew components (driven by vertical steering) when the steering multipoles 
  were specified without the dipole term. In that case, the skew component levels were essentially random values.
\end{itemize}

\subsection{Bug Fixes for Commands}
\begin{itemize}
\item Fixed two bugs in trajectory correction (\verb|correct| command): The range for threading mode was incorrectly set to the corrector tweak
  value instead of the corrector strength limit.  In the ``use actual beam''  mode of trajectory correction, trajectory correction is no longer aborted 
  if beam is lost during tracking.
\item Steering strength limit functions were not working correctly for unusual correction elements such as integrated steering coils on quadrupoles.
  This was reported by V. Sajaev (APS).
\item The \verb|link_elements| command now behaves as closer to what would be expected when links are used together with errors. 
  This addresses some issues identified by V. Sajaev (APS).
\item Included additional checks for valid ordering of commands, in order to prevent errors.
\end{itemize}

\subsection{New and Modified Elements}
\begin{itemize}
\item The \verb|WIGGLER| and \verb|CWIGGLER| elements are now included in beam moments calculations with the \verb|moments_output| command.
\end{itemize}

\subsection{New and Modified Commands}
\begin{itemize}
\item The \verb|load_parameters| command will now accept lists of patterns for matching names, types, and parameters in the input file.
\item The \verb|link_elements| command includes a new source-position specifier, namely, ``first.'' Also, a new feature to block self links was
  added, and is on by default. 
\item When performing threading-based trajectory correction, the algorithm now works backwards from the loss point and tries a specified number of correctors, which 
  improves performance. 
\item Quadratic spacing in the transverse coordinates is now the default for the \verb|tune_footprint| command.
\end{itemize}
...
\begin{itemize}
\item {\tt analyzeMagnets}: fixed divide-by-zero issue.
\item {\tt elasticScatteringLifetime}: Use different integration method to avoid issues with Gill-Miller when s values have an irregular distribution.
\item {\tt beamLifetimeCalc}: Use bremsstrahlung cross section from \cite{Tsai-RMP46}  and separation into nuclear and electron terms 
  following \cite{Wrulich-CAS94}. Previously, used erroneous expressions from \cite{LeDuff-NIM239}.
\item {\tt bremsstrahlungLifetime}: Use different integration method to avoid issues with Gill-Miller when s values have an irregular distribution.
  Use bremsstrahlung cross section from \cite{Tsai-RMP46}  and separation into nuclear and electron terms 
  following \cite{Wrulich-CAS94}. Previously, used erroneous expressions from \cite{LeDuff-NIM239}.
\item {\tt longitCalcs}: added option for a harmonic bunch-lenghtening cavity.
\item {\tt sddsrandmult}: now possible to have reference radius different from the magnet bore radius.
\item {\tt touschekLifetime}: if momentum aperture is zero, returns zero lifetime instead of running forever.
\end{itemize}
