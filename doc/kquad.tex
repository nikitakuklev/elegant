...
\item Floating point columns {\tt an} (or {\tt normal}) and {\tt bn} (or {\tt skew}) giving the values for the
...
\item Floating point column {\tt an} or {\tt normal} giving the values for the normal
...
The {\tt bn} values (for vertical steering) are deduced from the {\tt an} values, specifically,
$b_n = a_n*(-1)^{n/2}$.
\end{enumerate}
