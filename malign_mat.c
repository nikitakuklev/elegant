...
(
 ...
 short bodyCentered                     /* method=1: if non-zero, misalignment is relative to element central coordinates,
                                           otherwise, relative to entrance face */
 )
{
  ...
  if (method==0) {
    ...
    free_matrices(tmp); tfree(tmp);
    free_matrices(MEntrance); tfree(MEntrance);
    free_matrices(MExit); tfree(MExit);
  }
 ...
 )
{
  ...
  static MATRIX *RB, *RBT, *RXT, *OOp, *OpPp, *MTilt;
  ...
  xAxis->a[0][0] = 1; xAxis->a[1][0] = 0; xAxis->a[2][0] = 0; 
  yAxis->a[0][0] = 0; yAxis->a[1][0] = 1; yAxis->a[2][0] = 0; 
  zAxis->a[0][0] = 0; zAxis->a[1][0] = 0; zAxis->a[2][0] = 1;

  /* displacement-error vector in the xyz coordinate system */
  dVector->a[0][0] = dx; dVector->a[1][0] = dy; dVector->a[2][0] = dz;
  if (!m_copy(OP, dVector))
    bombElegant("m_copy(OP, dVector);", NULL);

  /* rotational-error matrix in the xyz coordinate system */
  cx = cos(ax); cy = cos(ay); cz = cos(az+tilt);
  sx = sin(ax); sy = sin(ay); sz = sin(az+tilt);
  ...
  if (thetaBend!=0 && tilt!=0) {
    MTilt->a[0][0] = cos(tilt);
    MTilt->a[0][1] = -sin(tilt);
    MTilt->a[0][2] = 0;
    MTilt->a[1][0] = sin(tilt);
    MTilt->a[1][1] = cos(tilt);
    MTilt->a[1][2] = 0;
    MTilt->a[2][0] = 0;
    MTilt->a[2][1] = 0;
    MTilt->a[2][2] = 1;
  ...
  if (face==1) {
    ...
    if (thetaBend!=0 && tilt!=0) {
      if (!m_mult(M2, MTilt, RB))
        bombElegant("m_mult(M2, M1, RB));", NULL);
      if (!m_copy(RB, M2))
        bombElegant("m_copy(RB, M2);", NULL);
    }
    ...
    if (thetaBend!=0 && tilt!=0) {
      if (!m_mult(V1, MTilt, OPp))
        bombElegant("m_mult(V1, MTilt, OPp));", NULL);
      m_copy(OPp, V1);
    }
    if (!m_mult(V1, RX, OPp))
 ...
 )
{
  ...
  static MATRIX *M1, *M2; /* working 3x3 matrices */
  static MATRIX *t0, *t1, *t2; /* working 6x1 matrices */
  static MATRIX *RB, *RBT, *RXT, *OOp, *OpPp, *OO0, *P0P, *MTilt;
  static short initialized = 0;
  double LD=0, Rc;
  long ip, i, j;
  static VMATRIX *LVM;
  ...
  /* displacement-error vector in the xyz coordinate system */
  d0Vector->a[0][0] = dx0; d0Vector->a[1][0] = dy0; d0Vector->a[2][0] = dz0;
 ...
 )
{
  ...
  static MATRIX *M1, *M2; /* working 3x3 matrices */
  static MATRIX *t0, *t1, *t2; /* working 6x1 matrices */
  static MATRIX *R0, *RB, *RB2, *RBT, *RXT, *OOp, *OpPp, *OO0, *O0P0, *P0P, *MTilt;
  static short initialized = 0;
  double LD = 0;
  long ip;
  double sinThetaW, cosThetaW, tanThetaW;
