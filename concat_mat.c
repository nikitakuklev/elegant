...
#define SYMM_C_C_Q(C, Q, j, k, l, n, m, p) (n >= m && m >= p ? C[j] * C[k] * Q[l][n][m][p] + C[j] * C[l] * Q[k][n][m][p] + \
                                                                 C[l] * C[k] * Q[j][n][m][p]                               \
                                                             : 0)
#define SYMM_C_C_T(C, T, j, k, l, n, m) (n >= m ? C[j] * C[k] * T[l][n][m] + C[j] * C[l] * T[k][n][m] + \
                                                    C[k] * C[l] * T[j][n][m]                            \
                                                : 0)
#define SYMM_C_R_T(C, R, T, j, k, l, n, m, p) (m >= p ? C[j] * R[k][n] * T[l][m][p] + C[k] * R[l][n] * T[j][m][p] + C[l] * R[j][n] * T[k][m][p] + \
                                                          C[j] * R[l][n] * T[k][m][p] + C[k] * R[j][n] * T[l][m][p] + C[l] * R[k][n] * T[j][m][p] \
                                                      : 0)
      ...
      for (m = 0; m < 6; m++) {
        ...
        for (j = sum = 0; j < 6; j++) {
          for (k = 0; k <= j; k++) {
            ...
            if (M1->order > 2) {
              for (l = 0; l <= k; l++) {
                sum += Q1[i][j][k][l] * (C0[j] * C0[k] * R0[l][m] +
                                         C0[k] * C0[l] * R0[j][m] +
                                         C0[l] * C0[j] * R0[k][m]);
              }
            }
          ...
          for (j = sum = 0; j < 6; j++) {
            ...
            if (M1->order > 1) {
              for (k = 0; k <= j; k++) {
                ...
                if (M1->order > 2) {
                  for (l = 0; l <= k; l++) {
                    sum += Q1[i][j][k][l] * (C0[j] * R0[k][m] * R0[l][n] +
                                             C0[j] * R0[k][n] * R0[l][m] +
                                             C0[k] * R0[l][m] * R0[j][n] +
                                             C0[k] * R0[l][n] * R0[j][m] +
                                             C0[l] * R0[j][m] * R0[k][n] +
                                             C0[l] * R0[j][n] * R0[k][m]);
                    if (M0->order > 1)
                      sum += Q1[i][j][k][l] * (SYMM_C_C_T(C0, T0, j, k, l, n, m) +
        ...
        for (m = 0; m <= n; m++) {
          for (p = 0; p <= m; p++) {
            ...
            if (M0->order > 1 && M1->order > 1) {
              for (j = 0; j < 6; j++) {
                for (k = 0; k <= j; k++) {
                  sum += T1[i][j][k] * ((n >= p ? SYMM_R_T(R0, T0, j, k, m, n, p) : 0) +
            ...
            if (M1->order > 2) {
              for (j = 0; j < 6; j++) {
                for (k = 0; k <= j; k++) {
                  for (l = 0; l <= k; l++) {
                    sum += Q1[i][j][k][l] * (R0[j][m] * R0[k][n] * R0[l][p] +
                    ...
                    if (M0->order > 1) {
                      sum += Q1[i][j][k][l] * (SYMM_C_R_T(C0, R0, T0, j, k, l, n, m, p) +
                    ...
                    if (M0->order > 2) {
                      sum += Q1[i][j][k][l] * (SYMM_C_C_Q(C0, Q0, j, k, l, n, m, p) +
            ...
            if (M0->order > 2 && M1->order > 1) {
              for (j = 0; j < 6; j++) {
                for (k = 0; k <= j; k++) {
                  sum += T1[i][j][k] * ((n >= m && m >= p ? SYMM_C_Q(C0, Q0, j, k, n, m, p) : 0) +
