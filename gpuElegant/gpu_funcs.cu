...
class gpu_rotateBeamCoordinates_kernel
{
 ...
 gpu_rotateBeamCoordinates_kernel(double cos_a, double sin_a) : cos_a(cos_a), sin_a(sin_a){};
...
class gpu_offsetBeamCoordinates_kernel
{
 ...
 gpu_offsetBeamCoordinates_kernel(double dx, double dy, double dz) : dx(dx), dy(dy), dz(dz){};
...
class gpu_center_beam_kernel
{
 ...
 gpu_center_beam_kernel(double o0, double o1, double o2, double o3,
                        double o4, double o5, double o6, double *d_timeCoord) : d_timeCoord(d_timeCoord)
  {
    offset[0] = o0;
    offset[1] = o1;
    offset[2] = o2;
...
class gpu_offset_beam_kernel
{
 ...
 gpu_offset_beam_kernel(double dx, double dy, double dz,
                        ...
                        double P_central, bool allParticles, long startPID, long endPID) : dx(dx), dy(dy), dz(dz), dxp(dxp), dyp(dyp), dt(dt), de(de),
...
class do_match_energy_P_average_transform
{
 public:
  double P_central;
 do_match_energy_P_average_transform(double P_central) : P_central(P_central){};
...
class do_match_energy_match_energy
{
 public:
  double dP_centroid;
  double P_central;
 do_match_energy_match_energy(double dP_centroid, double P_central) : dP_centroid(dP_centroid), P_central(P_central){};
...
class gpu_set_central_momentum_kernel
{
 ...
 gpu_set_central_momentum_kernel(double P_central, double P_new) : P_central(P_central), P_new(P_new){};
