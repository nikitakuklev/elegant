...
void recordTimings(char *name, float gputime, float cputime)
{
  ...
  timerListType *timer_list = getTimerList();
  if (timer_list->name->compare("none") == 0)
    *timer_list->name = name;
...
#  define COMPIND 6 /* use the last component */
  if (((double *)p1)[COMPIND] < ((double *)p2)[COMPIND])
    return -1;
  if (((double *)p1)[COMPIND] > ((double *)p2)[COMPIND])
    return 1;
  return 0;
  ...
  for (unsigned int ip = 0; ip < n_part; ip++)
    {
      for (unsigned int ic = 0; ic < n_comp; ic++)
        {
          if (isnan(h_particles[ip][ic]))
            cpuNaN = true;
          if (isnan(doh_particles[ip][ic]))
            gpuNaN = true;
          if (std::fabs(h_particles[ip][ic]) > mindbl)
            {
              residual = std::fabs((doh_particles[ip][ic] - h_particles[ip][ic]) / h_particles[ip][ic]);
              if (residual > tol)
                {
                  ndiff++;
                  avgdiff += residual;
...
void copyReductionArrays(double *centroid, double *sigma)
{
  ...
  for (unsigned int ii = 0; ii < NCOMP; ii++)
    {
      if (centroid)
        gpu_centroid[ii] = centroid[ii];
      if (sigma)
        gpu_sigma[ii] = sigma[ii];
    }
}
