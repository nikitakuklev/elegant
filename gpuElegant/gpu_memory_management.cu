...
void allocateGpuParticles(double** ptr_to_d_array){
  ...
  if(np < MINIMUM_ARRAY_SIZE) np = MINIMUM_ARRAY_SIZE;
  size_t pitch = 0;
  double* d_array;
...
void sharedTranspose(double* d_SoA, unsigned int d_SoA_pitch, double* d_AoS,
  ...
  sharedTransposeKernel<<<dimGrid,dimBlock, sharedMemorySize>>>
    (d_SoA, d_SoA_pitch, d_AoS, d_AoS_pitch, n_part, n_comp);
}
...
void sharedInverseTranspose(double* d_AoS, unsigned int d_AoS_pitch,
  ...
  sharedInverseTransposeKernel<<<dimGrid, dimBlock, sharedMemorySize>>>
    (d_AoS, d_AoS_pitch, d_SoA, d_SoA_pitch, n_part, n_comp);
}
...
void copyParticlesFromCpuToGpu(unsigned int n_part){
  ...
  if(gpubase->particlesOnGpu == 1) return;
  if(n_part <= 0) return;
...
void copyParticlesFromGpuToCpu(unsigned int n_part){
  ...
  if(gpubase->doh_particles != NULL) {
    ...
    h_particles = gpubase->accepted; 
    d_particles = gpubase->d_accepted;
  } else if(!gpubase->particlesOnGpu && !gpubase->copyForValidation) return;
