...
__global__ void gpu_rms_kernel(double* d_particles, unsigned int particlePitch,
  ...
  double r_v1 = s_v1[tx];
  reduceBlock<double, NTHREADS, Add<double> >
      (s_v1, r_v1, tx, NTHREADS, Add<double>() );
  double r_v2 = s_v2[tx];
  reduceBlock<double, NTHREADS, Add<double> >
      (s_v2, r_v2, tx, NTHREADS, Add<double>() );
  if(S11){
    double r_s11 = s_s11[tx];
    reduceBlock<double, NTHREADS, Add<double> >
        (s_s11, r_s11, tx, NTHREADS, Add<double>() );
  }
  if(S12){
    double r_s12 = s_s12[tx];
    reduceBlock<double, NTHREADS, Add<double> >
        (s_s12, r_s12, tx, NTHREADS, Add<double>() );
  }
  if(S22){
    double r_s22 = s_s22[tx];
    reduceBlock<double, NTHREADS, Add<double> >
        (s_s22, r_s22, tx, NTHREADS, Add<double>() );
  }
...
void gpu_rms_emittance(unsigned int i1, unsigned int i2, unsigned int n,
  ...
  if(s11 && s12 && s22)
    gpu_rms_kernel<true,true,true,nTx><<<grid,block>>>
      (d_particles,particlePitch,i1,i2,d_tempf,n);
  else if(s11 && s12 && !s22)
    gpu_rms_kernel<true,true,false,nTx><<<grid,block>>>
      (d_particles,particlePitch,i1,i2,d_tempf,n);
  else if(s11 && !s12 && s22)
    gpu_rms_kernel<true,false,true,nTx><<<grid,block>>>
      (d_particles,particlePitch,i1,i2,d_tempf,n);
  else if(s11 && !s12 && !s22)
    gpu_rms_kernel<true,false,false,nTx><<<grid,block>>>
      (d_particles,particlePitch,i1,i2,d_tempf,n);
  else if(!s11 && s12 && s22)
    gpu_rms_kernel<false,true,true,nTx><<<grid,block>>>
      (d_particles,particlePitch,i1,i2,d_tempf,n);
  else if(!s11 && s12 && !s22)
    gpu_rms_kernel<false,true,false,nTx><<<grid,block>>>
      (d_particles,particlePitch,i1,i2,d_tempf,n);
  else if(!s11 && !s12 && s22)
    gpu_rms_kernel<false,false,true,nTx><<<grid,block>>>
      (d_particles,particlePitch,i1,i2,d_tempf,n);
  else 
    gpu_rms_kernel<false,false,false,nTx><<<grid,block>>>
      (d_particles,particlePitch,i1,i2,d_tempf,n);
