                                        ...
                                        unsigned int *retirementCount)
{
  ...
  if (threadIdx.x == 0)
    {
      ...
      //s_blockHistogram_1 = (volatile double*)&s_blockHistogram_0[nb_height * nb_pitch];
    }
  __syncthreads();
  ...
  reduceBlock<double, 2048, Add<double> >(s_blockHistogram_0, unbinnedf, threadIdx.x, blockDim.x, Add<double>());
                                                  ...
                                                  unsigned int *d_unbinned, unsigned int *retirementCount)
{
  ...
  double unbinnedf = unbinned;
  reduceBlock<double, NB_PER_BLOCK, Add<double> >(s_blockHistogram_0, unbinnedf, threadIdx.x, blockDim.x, Add<double>());
                                             ...
                                             const double &dy, const int &xPower, const int &yPower, double *d_pz)
  {
    ...
    if (nb > nb_decomp_threshold)
    ...
    else
      {
        ...
        gpu_binTransverseKernel<<<dimGrid, dimBlock, sharedMemoryInBytes>>>(d_Itime0, d_Itime1, nb, d_particles, particlePitch, np,
