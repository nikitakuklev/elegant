...
class gpuComputeTimeCoordinatesFunctor
{
 public:
  double *d_time;
  double Po;
 gpuComputeTimeCoordinatesFunctor(double *time, double _Po) : d_time(time), Po(_Po) {}
...
#  endif
    ...
    { /* This serial part can be removed after all the upper level functions (e.g. wake) are parallelized */
#  ifndef USE_KAHAN
      tmean = gpuParticleReduction(np,
                                   gpuComputeTimeCoordinatesFunctor(d_time, Po), Add<double>());
#  else
...
class gpuComputeDistanceCoordinatesFunctor
{
 public:
  double *d_time;
  double Po;
 gpuComputeDistanceCoordinatesFunctor(double *d_time, double Po) : d_time(d_time), Po(Po) {}
