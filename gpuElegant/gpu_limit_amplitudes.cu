...
class gpu_rectangular_collimator_kernel1
{
 ...
 gpu_rectangular_collimator_kernel1(unsigned int *d_sortIndex, double xsize,
                                    ...
                                    double z, double Po, long openCode) : d_sortIndex(d_sortIndex), xsize(xsize), ysize(ysize), x_center(x_center),
...
class gpu_rectangular_collimator_kernel2
{
 ...
 gpu_rectangular_collimator_kernel2(unsigned int *d_sortIndex, double xsize,
                                    ...
                                    double z, double Po, long openCode, int invert) : d_sortIndex(d_sortIndex), xsize(xsize), ysize(ysize), x_center(x_center),
...
class gpu_limit_amplitudes_kernel
{
 ...
 gpu_limit_amplitudes_kernel(unsigned int *d_sortIndex, double xmax,
                             double ymax, double z, double Po, long extrapolate_z, long openCode) : d_sortIndex(d_sortIndex), xmax(xmax), ymax(ymax), z(z), Po(Po),
...
class gpu_removeInvalidParticles_kernel
{
 ...
 gpu_removeInvalidParticles_kernel(unsigned int *d_sortIndex,
                                   double z, double Po) : d_sortIndex(d_sortIndex), z(z), Po(Po){};
...
class gpu_elliptical_collimator_kernel1
{
 ...
 gpu_elliptical_collimator_kernel1(unsigned int *d_sortIndex, double z,
                                   ...
                                   long ye, double a2, double b2, long openCode) : d_sortIndex(d_sortIndex), z(z), Po(Po), xsize(xsize), ysize(ysize),
  ...
  __device__ unsigned int operator()(gpuParticleAccessor &part)
  {
    ...
    if ((pow(xo, xe) / a2 + pow(yo, ye) / b2) > 1)
      lost = openCode ? evaluateLostWithOpenSidesDevice(openCode, xo, yo, xsize, ysize) : 1;
    else if (isinf(part[0]) || isinf(part[2]) ||
             isnan(part[0]) || isnan(part[2]))
      lost = 1;
    if (lost)
...
class gpu_elliptical_collimator_kernel2
{
 ...
 gpu_elliptical_collimator_kernel2(unsigned int *d_sortIndex, double z,
                                   ...
                                   double dy, long exponent, long openCode, int invert) : d_sortIndex(d_sortIndex), z(z), Po(Po), xsize(xsize), ysize(ysize),
...
class gpu_elimit_amplitudes_kernel
{
 ...
 gpu_elimit_amplitudes_kernel(unsigned int *d_sortIndex, double z,
                              ...
                              double a2, double b2, long extrapolate_z, long openCode) : d_sortIndex(d_sortIndex), z(z), Po(Po), xmax(xmax), ymax(ymax),
...
class gpu_beam_scraper_kernel1
{
 ...
 gpu_beam_scraper_kernel1(int do_x, int do_y, double dx, double dy, double position,
                          ...
                          unsigned int *d_sortIndex, curandState_t *d_state, long *dflag) : do_x(do_x), do_y(do_y), dx(dx), dy(dy), position(position), length(length),
...
class gpu_beam_scraper_kernel2
{
 ...
 gpu_beam_scraper_kernel2(int do_x, int do_y, double dx, double dy, double position,
                          double Po, double z, unsigned int *d_sortIndex, long *dflag) : do_x(do_x), do_y(do_y), dx(dx), dy(dy), position(position), Po(Po), z(z),
...
class gpu_beam_scraper_kernel3
{
 ...
 gpu_beam_scraper_kernel3(int do_x, int do_y, double dx, double dy, double position,
                          double Po, double z, unsigned int *d_sortIndex, long *dflag) : do_x(do_x), do_y(do_y), dx(dx), dy(dy), position(position), length(length), Po(Po),
...
class gpu_imposeApertureData_kernel
{
 ...
 gpu_imposeApertureData_kernel(double xCenter, double yCenter, double xSize,
                               double ySize, double z, double Po, unsigned int *d_sortIndex) : xCenter(xCenter), yCenter(yCenter), xSize(xSize), ySize(ySize),
