...
#else
        if (myid == 1)
          { /* If the first particle is lost, Pelegant and elegant will not have the same fiducial time */
            if (np)
              tFid = (coord[4] + sOffset) / (c_mks * beta_from_delta(p0, np ? coord[5] : 0.0));
            else
...
class gpuRfTracFunction2 : public gpuFunctionIfc
{
 public:
  double dx;
  double dy;
 gpuRfTracFunction2(const double &adx, const double &ady) : dx(adx), dy(ady){};
...
{
 public:
 gpuRfTracFunction4(const double &alength, const long &aend2Focus, const long &akickInd, const long &anKicks, double *ad_inverseF) : length(alength),
...
class gpuRfTracFunction5 : public gpuFunctionIfc
{
 ...
 gpuRfTracFunction5(const double &alength, const double &aPcentral,
                    ...
                    double *ad_dgammaOverGammaAve) : length(alength), pcentral(aPcentral), cmks(ac_mks),
