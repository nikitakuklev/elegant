  ...
  for (ip = 0; ip < np; ip++) {
    ...
    if (abs(higherOrder) > 1) {
      ...
      if (abs(higherOrder) > 2) {
        ...
        if (higherOrder > 0) {
          dx = (a * x * (8 * (xpow[2] + 3 * ypow[2]) + 4 * apow[2] * (5 * xpow[6] + 21 * xpow[4] * ypow[2] - 25 * xpow[2] * ypow[4] - ypow[6]) + apow[3] * (35 * xpow[8] + 84 * xpow[6] * ypow[2] + 498 * xpow[4] * ypow[4] - 108 * xpow[2] * ypow[6] + 3 * ypow[8]) + 12 * a * ipow(xpow[2] - ypow[2], 2))) / 8;
          dy = (a * y * (-8 * (3 * xpow[2] + ypow[2]) + 4 * apow[2] * (xpow[6] + 25 * xpow[4] * ypow[2] - 21 * xpow[2] * ypow[4] - 5 * ypow[6]) + apow[3] * (3 * xpow[8] - 108 * xpow[6] * ypow[2] + 498 * xpow[4] * ypow[4] + 84 * xpow[2] * ypow[6] + 35 * ypow[8]) + 12 * a * ipow(xpow[2] - ypow[2], 2))) / 8;
        }
      } else {
        for (i = 1; i < 4; i++) {
          xpow[i] = xpow[i - 1] * x;
