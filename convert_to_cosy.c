                    ...
                    double hgap, double fint)
{
  ...
  if (angle<0) 
    fprintf(fp, "    CB ;\n");
  fprintf(fp, "    FR 3 ;\n");
  fprintf(fp, "    FC 1 1 1 %.15g %.15g %.15g 0.0 0.0 0.0 ; \n",
          engeCoef[0], engeCoef[1], engeCoef[2]);
  fprintf(fp, "    FC 1 2 1 %.15g %.15g %.15g 0.0 0.0 0.0 ; \n",
          engeCoef[0], engeCoef[1], engeCoef[2]);
  fprintf(fp, "    MSS %.15g %.15g %.15g %.15g %.15g %.15g %.15g W; \n",
                     ...
                     double quadBoreRadius, double sextBoreRadius)
{
    ...
    /* user-defined order, 6D phase space, no knobs */
    fprintf(fp, "OV %ld 3 0 ;\n", cosyOrder);
    /* electrons with given momentum */
    fprintf(fp, "RPE %.15g ;\n", (sqrt(pCentral*pCentral+1)-1)*me_mks*sqr(c_mks)/e_mks/1e6);
    /* initialize map to unity */
    fprintf(fp, "UM ;\n");
