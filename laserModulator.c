...
void trackLaserModulator(double **part, long nPart, LSRMDLTR *lsrMdltr, double Po)
{
  ...
  long N, harm, np, i, j;
  double gam0, E0, xl0, yl0, ku, lam;
  double per, ku, K, theta0, Lb, ds, R;
  double xpk, k, w, Ef0, ZR, w0, P0, *sr, *theta, *C, *S;
  VMATRIX *Rp, *Rm, *R1;
  COMPLEX Q, Efx, ctmp1, ctmp2, ctmp3;
  double Efz, Bfx, Bfy, Bfz;
  double Bf[3], Ef[3], v[3], F[3], dkik[3];
  double tmp1, tmp2, tmp3;
  
  /* force number of periods to be even */
  N = 2*(lsrMdltr->periods/2);
  per = lsrMdltr->length/N;
  ...
  Lb     = period/2*(1 + 1/6*sqr(K/gam0));  /*  % length of arc over an und. half-period [m] */
  ...
  for (jN=0; jN<(2*Nper+1); jN++) {    /* % loop over each period */
    for (j=0; j<np; j++) {            /* % loop within each period */
      for (i=0; i<nPart; i++) {         /* loop over all particles */
        ...
        ctmp1 = cmulr(cmul(Q, cassign(0, 1)), -k);
        ctmp2 = cadd(cadd(cmulr(cipowr(Q, 2), sqr(x)), ctmp1), cassign(0, 1));
        Bfy = cmul(Efx, ctmp2);
